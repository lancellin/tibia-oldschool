...
local cancelWalkEvent

local WALK_INPUT_DELAY = 15
local WALK_CANCEL_DELAY = 15
local WALK_TURN_DELAY_REPEATED = 15
local WALK_TURN_DELAY_DEFAULT = 15
...
local function walk(dir)
    ...
    if isAutoWalking then
        g_game.stop()
        player:stopAutoWalk()
        player:lockWalk(player:getStepDuration() + WALK_INPUT_DELAY)
        return
    end
    ...
    if not player:canWalk() then
        if player:isWalking() or lastWalkDir ~= dir then
            nextWalkDir = dir
        end
        return
    end
...
local function turn(dir, repeated)
    ...
    local delay = repeated and WALK_TURN_DELAY_REPEATED or WALK_TURN_DELAY_DEFAULT

    if lastTurn + delay < g_clock.millis() then
        g_game.turn(dir)
        changeWalkDir(dir)
        lastTurn = g_clock.millis()
        player:lockWalk(math.max(g_settings.getNumber("walkTurnDelay"), WALK_INPUT_DELAY))
    end
end

--- Binds movement keys to their respective directions.
local function bindKeys()
    modules.game_interface.getRootPanel():setAutoRepeatDelay(WALK_INPUT_DELAY)
...
local function onTeleport(player, newPos, oldPos)
    ...
    local TELEPORT_DELAY = math.max(g_settings.getNumber("walkTeleportDelay"), WALK_INPUT_DELAY)
    local STAIRS_DELAY = 0
...
local function onWalkFinish(player)
    if nextWalkDir then
        if not g_game.getFeature(GameAllowPreWalk) then
            ...
            addWalkEvent(nextWalkDir, WALK_INPUT_DELAY)
        end
    end
end
...
local function onCancelWalk(player)
    player:lockWalk(WALK_CANCEL_DELAY)
end
...
function bindWalkKey(key, dir)
    ...
    g_keyboard.bindKeyDown(key, function()
        g_keyboard.setKeyDelay(key, WALK_INPUT_DELAY)
        changeWalkDir(dir)
...
function bindTurnKey(key, dir)
    ...
    g_keyboard.bindKeyUp(key, function()
        local player = g_game.getLocalPlayer()
        if player then player:lockWalk(WALK_INPUT_DELAY) end
    end, gameRootPanel)
end
