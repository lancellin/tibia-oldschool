...
local automaticContainerSortingEnabled = false
local loginContainerRestorePhase = true

local function onContainerRestoreState(protocolGame, opcode, buffer)
    if buffer == 'end' then
        loginContainerRestorePhase = false
    end
end

local function onGameStart()
    loginContainerRestorePhase = true
end
...
local function enforceManualContainerOrder()
    if not containerSettings then
        containerSettings = {}
    end

    local changed = containerSettings['useManualSortMode'] ~= 1 or
                        containerSettings['currentSortMode'] ~= 'none' or
                        containerSettings['sortContainersFirst'] ~= 0 or
...
function init()
    ...
    ProtocolGame.registerExtendedOpcode(ExtendedIds.ContainerRestore, onContainerRestoreState)
...
function terminate()
    ProtocolGame.unregisterExtendedOpcode(ExtendedIds.ContainerRestore)
...
function sortContainerItems(container, sortMode)
    if not automaticContainerSortingEnabled then
        return
    end
...
function onContainersMenuAction(actionId)
    local isAutomaticSortingAction = actionId == 'sortContainersFirst' or
                                         actionId == 'sortNestedContainers' or
                                         actionId == 'useManualSortMode' or
                                         actionId:find('sortAsc') or
                                         actionId:find('sortDesc')
    if not automaticContainerSortingEnabled and isAutomaticSortingAction then
        enforceManualContainerOrder()
        return
    end
...
function toggleContainerPages(containerWindow, pages)
    ...
    if pages then
        ...
        -- When pages are active, move upButton next to the minimize button.
        if upButton and upButton:isVisible() and minimizeButton then
            ...
            -- Position lockButton directly to the left of upButton.
            if lockButton then
        ...
        if upButton and minimizeButton then
...
function onContainerOpen(container, previousContainer)
    local restoreSavedLayout = loginContainerRestorePhase
    ...
    -- Automatic sorting is disabled, so its menu must not be exposed.
    ...
    containerWindow.onMousePress = function(widget, mousePos, mouseButton)
        ...
        if containerWindow:isLocked() then
    ...
    containerWindow.onMouseRelease = function(widget, mousePos, mouseButton)
        if not containerWindow:isLocked() then
    ...
    if restoreSavedLayout then
        -- Keep the proven login restoration path exactly as it was. It is the
        -- only path allowed to restore saved CID positions and order panels.
        containerWindow:eraseSettings({
            closed = true
        })
        containerWindow:setupOnStart()
    elseif not previousContainer then
        -- A container opened during the session stays where it was appended.
        -- Save only this window: never reorder or rewrite the other movable
        -- miniwindows (map, stats, buttons, battle list, etc.).
        ...
        if parent and parent:getClassName() == 'UIMiniWindowContainer' then
            containerWindow:saveParentIndex(parent:getId(), parent:getChildIndex(containerWindow))
        end
        containerWindow:fitOnParent()
    end
