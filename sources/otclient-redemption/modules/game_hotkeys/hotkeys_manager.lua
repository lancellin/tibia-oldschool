...
local function isForegroundWindowOpen()
    ...
    -- The top menu is part of the regular game interface, not a foreground window.
    ...
    -- Custom foreground interfaces (for example HTML windows) may not use a
    -- window-named OTUI style, but they still take the root focus while in use.
    local focusedRootChild = rootWidget:getFocusedChild()
    return focusedRootChild and focusedRootChild ~= gameRootPanel and focusedRootChild ~= topMenu
end
...
function canPerformKeyCombo(keyCombo)
    ...
    if isForegroundWindowOpen() then
        return false
    end
    if not modules.game_console.isChatEnabled() then
        return true
