...
function notificationsController:showBestiaryUnlock(outfit, creatureName)
    if not modules.client_options.getOption("showInfoBanner") then
        return
    end
    ...
    self:show(
        ...
        creatureName,
        "/game_notifications/assets/images/nodo/icon-infobanner-bestiary",
        DEFAULT_HOLD_MS,
        { outfit = outfit, hideIcon = true }
    )
end

function notificationsController:showBestiaryStage(outfit, creatureName, stage)
    if not modules.client_options.getOption("showInfoBanner") then
        return
    end
    ...
    local description
    if stage == 3 then
        description = string.format("%s - Last stage completed.", creatureName)
    else
        description = string.format("%s - Stage %d completed.", creatureName, stage)
    end
...
function notificationsController:showAdvance(serverSkillId, level)
    if serverSkillId == 8 then
        self:onClientEvent(eventCategory.CLIENT_EVENT_TYPE_LEVEL, level)
        return
    end

    local clientSkillId = serverSkillIds[serverSkillId]
    if not clientSkillId then
        g_logger.warning(string.format("Unknown server skill id for advance banner: %s", tostring(serverSkillId)))
        return
    end

    self:onClientEvent(eventCategory.CLIENT_EVENT_TYPE_SKILL, clientSkillId, level)
end
...
function notificationsController:processNext()
    ...
    self:resetBanner()
    if data.extraData and data.extraData.hideIcon then
        self.widgets.icon:hide()
    else
        self.widgets.icon:show()
    end
    if data.img and not (data.extraData and data.extraData.hideIcon) then
