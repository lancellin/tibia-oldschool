...
function refreshTradeItems()
    ...
    for key, item in pairs(currentTradeItems) do
        ...
        itemWidget:setItem(item.displayPtr or item.ptr)
...
function onOpenNpcTrade(items)
    ...
    for key, item in pairs(items) do
        if item[4] > 0 then
            ...
            if g_game.getClientVersion() == 772 and newItem.ptr:isFluidContainer() then
                local fluidName = newItem.name:lower()
                if fluidName == 'mana fluid' or fluidName == 'life fluid' then
                    newItem.displayPtr = newItem.ptr:clone()
                    newItem.displayPtr:setCount(fluidName == 'mana fluid' and 7 or 10)
                end
            end
