...
-- Bestiary loot difficulty, calculated per monster drop entry by the server.
-- Keep this separate from the global, value-based item rarity above: the same
-- item can have a different drop difficulty for different creatures.
ItemsDatabase.lootDifficultyColors = {
    [0] = TextColors.white,  -- Common
    [1] = TextColors.grey,   -- Uncommon
    [2] = TextColors.green,  -- Semi-Rare
    [3] = TextColors.blue,   -- Rare
    [4] = '#c678dd',         -- Very Rare
    [5] = TextColors.yellow  -- Extremely Rare
}
...
function ItemsDatabase.setColorLootMessage(text)
    local function coloringLootName(match)
        local id, difficulty, itemName = match:match("^(%d+)|(%d+)|(.+)$")
        if id and difficulty and itemName then
            local color = ItemsDatabase.lootDifficultyColors[tonumber(difficulty)] or TextColors.white
            return "{" .. itemName .. ", " .. color .. "}"
        end

        -- Compatibility with older/value-based messages generated by other
        -- client modules, which still use the {itemId|itemName} format.
        id, itemName = match:match("^(%d+)|(.+)$")
