...
function reloadMainPanelSizes()
    ...
    main_panel:setHeight(0)
    main_panel:setImageColor('alpha')

    local panel = optionsController and optionsController.ui
    if panel and panel.panelHeight ~= nil then
...
function optionsController:onGameStart()
    optionsController.ui:setupOnStart()
...
function toggleExtendedViewButtons(extended)
    ...
    if extended then
        ...
        if rightPanel:hasChild(optionsController.ui) then
            rightPanel:moveChildToIndex(optionsController.ui, 1)
        end
    end
    refreshOptionsSizes()
end
