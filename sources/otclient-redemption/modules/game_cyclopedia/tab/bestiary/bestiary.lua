...
local function getBestiaryDisplayData(data)
    if data and data.name and data.name ~= "" and data.outfit then
        return data.name, data.outfit
    end

    local raceData = data and g_things.getRaceData(data.id) or nil
    if raceData then
        return raceData.name, raceData.outfit
    end

    return "Unknown Creature", nil
end

local function formatBestiaryName(name)
    if not name or name == "" then
        return "Unknown Creature"
    end

    local lowerName = name:lower()
    return lowerName:gsub("(%a)([%w']*)", function(first, rest)
        return first:upper() .. rest
    end)
end

local function getValidBestiaryRenderOutfit(creatureName, creatureOutfit)
    if not creatureOutfit then
        g_logger.info(string.format("[Bestiary] missing outfit for '%s'", tostring(creatureName)))
        return nil
    end

    local outfitType = tonumber(creatureOutfit.type)
    if not outfitType or outfitType == 0 then
        g_logger.info(string.format(
            "[Bestiary] invalid outfit for '%s': type=%s outfit=%s",
            tostring(creatureName),
            tostring(creatureOutfit.type),
            tostring(creatureOutfit)))
        return nil
    end

    return creatureOutfit
end

local function isBestiaryCreatureDiscovered(data)
    return (tonumber(data and data.killCounter) or 0) > 0
end

local function getBestiaryUnlockedLootDifficulty(data)
    if not isBestiaryCreatureDiscovered(data) then
        return -1
    end

    local currentLevel = tonumber(data and data.currentLevel) or 0
...
function Cyclopedia.setBestiaryTrackerStatus(raceId, checked, trackerEntry, sendToServer)
    ...
    if not Cyclopedia.BestiaryOnlyMode and sendToServer ~= false then
        g_game.sendStatusTrackerBestiary(raceId, checked)
    end
end

function Cyclopedia.onBestiaryTrackCheckChange(widget)
    if Cyclopedia.BestiaryOnlyMode then
        setBestiaryTrackCheck(widget, false)
        return
    end
...
function Cyclopedia.loadBestiaryOverview(name, creatures, animusMasteryPoints)
    if (name == "Result" or name == "") and #creatures > 0 then
        ...
        Cyclopedia.Bestiary.CurrentCategoryName = name
        Cyclopedia.Bestiary.CurrentCategoryCreatures = creatures
    ...
    if name == "Creatures" or name == "Bosses" then
        Cyclopedia.storeBestiarySearchPool(name, creatures)
    end
end
...
function showBestiary()
    ...
    Cyclopedia.Bestiary.CategoryPage = 1
    Cyclopedia.Bestiary.CreaturesPage = 1
    Cyclopedia.Bestiary.SearchPage = 1
    Cyclopedia.Bestiary.DetailBackPage = 1
    controllerCyclopedia.ui.CharmsBase:setVisible(false)
    controllerCyclopedia.ui.GoldBase:setVisible(true)
    controllerCyclopedia.ui.BestiaryTrackerButton:setVisible(not Cyclopedia.BestiaryOnlyMode)
    if g_game.getClientVersion() >= 1410 then
        controllerCyclopedia.ui.CharmsBase1410:hide()
    end

    if Cyclopedia.BestiaryOnlyMode then
        UI.ListBase.CreatureInfo.LeftBase.TrackCheck:setVisible(false)
        UI.ListBase.CreatureInfo.LeftBase.TrackCheck:setEnabled(false)
    end
    ...
    Cyclopedia.Bestiary.AllCreaturesFlatRequested = false
    Cyclopedia.Bestiary.AllCreaturesFlat = {}
    Cyclopedia.Bestiary.SearchPools = {}
    Cyclopedia.Bestiary.HideUnknown = false
    Cyclopedia.Bestiary.CurrentCategoryName = nil
    Cyclopedia.Bestiary.CurrentCategoryCreatures = nil
    UI.HideUnknownCheck:setChecked(false)
...
function Cyclopedia.storeBestiarySearchPool(categoryName, data)
    Cyclopedia.Bestiary.SearchPools = Cyclopedia.Bestiary.SearchPools or {}
    Cyclopedia.Bestiary.SearchPools[categoryName] = {}

    for i = 1, #data do
        table.insert(Cyclopedia.Bestiary.SearchPools[categoryName], {
            id = data[i].id,
            name = data[i].name,
            outfit = data[i].outfit,
    ...
    for _, poolName in ipairs({ "Creatures", "Bosses" }) do
        for _, creature in ipairs(Cyclopedia.Bestiary.SearchPools[poolName] or {}) do
            table.insert(Cyclopedia.Bestiary.AllCreaturesFlat, creature)
        end
    end
end

local function getBestiaryFilteredCreatures(data)
    if not Cyclopedia.Bestiary.HideUnknown then
        return data
    end

    local filtered = {}
    for i = 1, #data do
        if (tonumber(data[i].currentLevel) or 0) > 0 then
...
local function centerBestiaryCreatureGrid(scrollArea, itemCount)
    if not scrollArea then
        return
    end

    if itemCount <= 0 then
        scrollArea:setPaddingLeft(5)
        return
    end

    local areaWidth = scrollArea:getWidth()
    if areaWidth <= 0 then
        scrollArea:setPaddingLeft(5)
        return
    end
    ...
    local centeredPadding = math.max(5, math.floor((areaWidth - usedWidth) / 2))

    scrollArea:setPaddingLeft(centeredPadding)
end

function Cyclopedia.onBestiaryHideUnknownChange(widget)
    Cyclopedia.Bestiary.HideUnknown = widget:isChecked()

    if Cyclopedia.Bestiary.Stage == STAGES.CREATURES and Cyclopedia.Bestiary.CurrentCategoryCreatures then
        Cyclopedia.Bestiary.Page = 1
        Cyclopedia.Bestiary.CreaturesPage = 1
        Cyclopedia.loadBestiaryCreatures(Cyclopedia.Bestiary.CurrentCategoryCreatures)
    end
end

local function getBestiaryPageKey(stage)
...
local function getBestiaryStoredPage(stage)
    local pageKey = getBestiaryPageKey(stage)
    if not pageKey then
        return 1
    end

    return math.max(1, tonumber(Cyclopedia.Bestiary[pageKey]) or 1)
end

local function setBestiaryStoredPage(stage, page)
    local pageKey = getBestiaryPageKey(stage)
    if pageKey then
        Cyclopedia.Bestiary[pageKey] = math.max(1, tonumber(page) or 1)
    end
end
...
function Cyclopedia.CreateCreatureItems(data, maxUnlockedDifficulty)
    ...
    for index = 0, 5 do
        ...
        elseif index == 4 then
            ...
            widget.Title:setText(tr("Extremely Rare") .. ":")
        end
        ...
        for itemIndex, itemData in ipairs(data[index]) do
            ...
            local isUnlocked = index <= maxUnlockedDifficulty

            itemWidget:setTooltip(isUnlocked and (itemData.name or "") or "")
            itemWidget.onMouseRelease = nil
            itemWidget.undefinedItem:setVisible(not isUnlocked or itemData.id == 0)

            if isUnlocked and itemData.id > 0 then
                itemWidget:setItemId(itemData.id)
                itemWidget.id = itemData.id
                local thing = g_things.getThingType(itemData.id, ThingCategoryItem)
                itemWidget.classification = thing and thing:getClassification() or 0
                ...
                itemWidget.id = 0
                itemWidget.classification = 0
                itemWidget.Stackable:setText("")
                itemWidget.Rarity:setImageSource("")
            end
...
function Cyclopedia.loadBestiarySelectedCreature(data)
    ...
    local creatureName, creatureOutfit = getBestiaryDisplayData(data)
    local formattedName = formatBestiaryName(creatureName)
    local isDiscovered = isBestiaryCreatureDiscovered(data)
    local maxUnlockedLootDifficulty = getBestiaryUnlockedLootDifficulty(data)

    creatureOutfit = getValidBestiaryRenderOutfit(creatureName, creatureOutfit)
    UI.ListBase.CreatureInfo:setText(isDiscovered and formattedName or "Unknown Creature")
    Cyclopedia.SetBestiaryDiamonds(occurence[data.ocorrence])
    Cyclopedia.SetBestiaryStars(data.difficulty)
    if creatureOutfit then
        UI.ListBase.CreatureInfo.LeftBase.Sprite:setOutfit(creatureOutfit)
        UI.ListBase.CreatureInfo.LeftBase.Sprite:getCreature():setStaticWalking(1000)
    end

    if isDiscovered and UI.ListBase.CreatureInfo.LeftBase.Sprite:getCreature() then
        UI.ListBase.CreatureInfo.LeftBase.Sprite:getCreature():setShader("")
    elseif UI.ListBase.CreatureInfo.LeftBase.Sprite:getCreature() then
    ...
    if isDiscovered then
        UI.ListBase.CreatureInfo.Value1:setText(tostring(data.maxHealth or "?"))
        UI.ListBase.CreatureInfo.Value2:setText(tostring(data.experience or "?"))
        UI.ListBase.CreatureInfo.Value3:setText(tostring(data.speed or "?"))
        UI.ListBase.CreatureInfo.Value4:setText(tostring(data.armor or "?"))
        UI.ListBase.CreatureInfo.Value5:setText(string.format("%s%%", tostring(data.mitigation or 0)))
        UI.ListBase.CreatureInfo.BonusValue:setText(tostring(data.charmValue or "?"))
    else
    ...
    if isDiscovered and data.attackMode == 1 then
        local rect = {
            height = 9,
            x = 18,
            y = 0,
    ...
    elseif isDiscovered then
        ...
        UI.ListBase.CreatureInfo.SubTextLabel:setSize("0 0")
    end
    ...
    if isDiscovered and not table.empty(data.combat) then
        for i = 1, 8 do
        ...
        for i = 1, 8 do
            ...
            UI.ListBase.CreatureInfo[resists[i]]:removeTooltip()
        end
    end
    ...
    Cyclopedia.CreateCreatureItems(lootData, maxUnlockedLootDifficulty)
    UI.ListBase.CreatureInfo.LocationField.Textlist.Text:setText(isDiscovered and (data.location or "") or "?????")
...
function Cyclopedia.openBestiaryCreatureDetail(raceId, backStage)
    ...
    Cyclopedia.Bestiary.DetailBackPage = Cyclopedia.Bestiary.Page or getBestiaryStoredPage(Cyclopedia.Bestiary.DetailBackStage)
...
function Cyclopedia.CreateBestiaryCategoryItem(Data)
    ...
    local iconPath = "/game_cyclopedia/images/bestiary/creatures/" .. Data.name:lower():gsub(" ", "_")
    if not g_resources.fileExists(iconPath .. ".png") then
        iconPath = "/game_cyclopedia/images/bestiary/creatures/-.---"
    end
...
function Cyclopedia.loadBestiarySearchCreatures(data)
    ...
    Cyclopedia.Bestiary.Page = getBestiaryStoredPage(STAGES.SEARCH)
    ...
    Cyclopedia.Bestiary.SearchPage = Cyclopedia.Bestiary.Page
...
function Cyclopedia.loadBestiaryCreatures(data)
    local filteredData = getBestiaryFilteredCreatures(data)
    Cyclopedia.Bestiary.Creatures = {}
    Cyclopedia.Bestiary.Page = getBestiaryStoredPage(STAGES.CREATURES)

    local maxCategoriesPerPage = 15
    Cyclopedia.Bestiary.TotalCreaturesPages = math.ceil(#filteredData / maxCategoriesPerPage)
    ...
    Cyclopedia.Bestiary.CreaturesPage = Cyclopedia.Bestiary.Page
    ...
    for i = 1, #filteredData do
        ...
        local creature = {
            ...
            outfit = filteredData[i].outfit,
            currentLevel = filteredData[i].currentLevel,
            AnimusMasteryBonus = filteredData[i].creatureAnimusMasteryBonus,
...
function Cyclopedia.BestiarySearch()
    ...
    for _, creature in ipairs(Cyclopedia.Bestiary.AllCreaturesFlat or {}) do
        local creatureName = (creature.name or ""):lower()
        if creatureName:find(query, 1, true) then
...
function Cyclopedia.CreateBestiaryCreaturesItem(data)
    ...
    local spriteCreature = nil
    if creatureOutfit then
        widget.Sprite:setOutfit(creatureOutfit)
        spriteCreature = widget.Sprite:getCreature()
    ...
    if data.currentLevel >= 4 then
        ...
        if data.currentLevel < 1 then
            ...
            widget.Name:setText("Unknown Creature")
            widget.AnimusMastery:setVisible(false)
        else
...
function Cyclopedia.loadBestiaryCreature(page, search)
    ...
    centerBestiaryCreatureGrid(UI.ListBase.CreatureList, #Cyclopedia.Bestiary[state][page])
...
function Cyclopedia.loadBestiaryCategories(data)
    ...
    for i = 1, #data do
        ...
        local category = {
            ...
            amount = math.max(data[i].count or 0, data[i].unlockedCount or 0),
            know = math.min(data[i].count or 0, data[i].unlockedCount or 0),
    ...
    if Cyclopedia.BestiaryOnlyMode and not Cyclopedia.Bestiary.AllCreaturesFlatRequested then
        Cyclopedia.Bestiary.AllCreaturesFlatRequested = true
        g_game.requestBestiaryOverview("Creatures", false, {})
        g_game.requestBestiaryOverview("Bosses", false, {})
    end
end
...
function Cyclopedia.onStageChange()
    ...
    if Cyclopedia.Bestiary.Stage == STAGES.CREATURE then
        ...
        function UI.BackPageButton.onClick()
            ...
            Cyclopedia.Bestiary.Page = Cyclopedia.Bestiary.DetailBackPage or getBestiaryStoredPage(Cyclopedia.Bestiary.Stage)
            setBestiaryStoredPage(Cyclopedia.Bestiary.Stage, Cyclopedia.Bestiary.Page)
            Cyclopedia.onStageChange()
            if Cyclopedia.Bestiary.Stage == STAGES.CREATURES then
                Cyclopedia.loadBestiaryCreature(Cyclopedia.Bestiary.Page, false)
            elseif Cyclopedia.Bestiary.Stage == STAGES.SEARCH then
                Cyclopedia.loadBestiaryCreature(Cyclopedia.Bestiary.Page, true)
            elseif Cyclopedia.Bestiary.Stage == STAGES.CATEGORY then
                Cyclopedia.loadBestiaryCategory(Cyclopedia.Bestiary.Page)
            end
        end
    end
