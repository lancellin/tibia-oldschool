...
local BATTLE_DISTANCE_BATCH_SMALL_MS = 70
local BATTLE_DISTANCE_BATCH_LARGE_MS = 120
local BATTLE_DISTANCE_BATCH_THRESHOLD = 10
...
function BattleListInstance:destroy(saveSettings)
    ...
    if self.battleDistanceUpdateEvent then
        removeEvent(self.battleDistanceUpdateEvent)
        self.battleDistanceUpdateEvent = nil
    end
...
local function getBattleDistanceBatchDelay(instance)
    return #instance.binaryTree >= BATTLE_DISTANCE_BATCH_THRESHOLD and BATTLE_DISTANCE_BATCH_LARGE_MS or BATTLE_DISTANCE_BATCH_SMALL_MS
end

local function applyBattleDistanceUpdate(instance)
    instance.battleDistanceUpdateEvent = nil

    if not instance.panel or not g_game.isOnline() then
        return
    end
    ...
    for _, v in ipairs(instance.binaryTree) do
        local battleButton = instance.battleButtons[v.id]
        local mob = battleButton and (battleButton.creature or g_map.getCreatureById(v.id))
        local mobPos = mob and mob:getPosition()
        if battleButton and battleButton.data and mobPos then
            local newDistance = getDistanceBetween(playerPos, mobPos)
            if v.distance ~= newDistance then
...
local function scheduleBattleDistanceUpdate(instance)
    if not instance or not instance.panel then
        return
    end

    if instance.battleDistanceUpdateEvent then
        return
    end

    local delay = getBattleDistanceBatchDelay(instance)
    instance.battleDistanceUpdateEvent = scheduleEvent(function()
        applyBattleDistanceUpdate(instance)
    end, delay)
end
            ...
            if creature:isLocalPlayer() then
                ...
                elseif oldPos and newPos and (newPos.x ~= oldPos.x or newPos.y ~= oldPos.y) then
                    ...
                    if #instance.binaryTree > 0 and sortType == 'distance' then
                        scheduleBattleDistanceUpdate(instance)
                    else
