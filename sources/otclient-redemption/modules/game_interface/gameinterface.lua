...
selectedThing = nil
selectedType = nil
local selectedCursorApplied = false
...
local containerPanelSelectors = {}

local function updateContainerPanelSelectors()
    if not gameRootPanel or gameRootPanel:isDestroyed() then
        return
    end

    local visibleIndex = 0
    for _, data in ipairs(containerPanelSelectors) do
        local panel = data.panel
        local checkbox = data.checkbox
        local visible = panel and checkbox and panel:isVisible() and panel:isOn() and
                            panel:getWidth() > 0 and panel:getHeight() > 0

        checkbox:setVisible(visible)
        if visible then
            visibleIndex = visibleIndex + 1
            checkbox:setText(tostring(visibleIndex))
            checkbox:setTooltip(tr('Open new containers in column %d', visibleIndex))
        end
    end
end

local function scheduleContainerPanelSelectorUpdate()
    addEvent(updateContainerPanelSelectors)
end
...
local function updateSidePanelButtons()
    ...
    scheduleContainerPanelSelectorUpdate()
end
...
function init()
    ...
    -- Display numbers follow the physical, currently visible columns from
    -- left to right. The saved selection still uses the stable panel id.
    containerPanelSelectors = { {
        panel = gameLeftPanel,
        checkbox = gameRootPanel:getChildById('gameSelectLeftColumn')
    }, {
        panel = gameLeftExtraPanel,
    ...
    for _, data in ipairs(containerPanelSelectors) do
        connect(data.panel, {
            onVisibilityChange = scheduleContainerPanelSelectorUpdate
        })
    end
    scheduleContainerPanelSelectorUpdate()
...
local function applySelectedCursor(force)
    if not selectedThing or not g_ui.isMouseGrabbed() then
        return
    end

    local nativeCursor = modules.client_options and modules.client_options.getOption('nativeCursor')
    if nativeCursor and selectedType ~= 'use' then
        g_window.setSystemCursor('cross')
        return
    end

    if force and selectedCursorApplied then
        g_mouse.popCursor('target')
        selectedCursorApplied = false
    end

    if not selectedCursorApplied then
        g_mouse.pushCursor('target')
        selectedCursorApplied = true
    end
end

local function clearSelectedCursor()
    local nativeCursor = modules.client_options and modules.client_options.getOption('nativeCursor')
    if selectedCursorApplied then
        g_mouse.popCursor('target')
        selectedCursorApplied = false
    end
...
function isMouseActionCursorActive()
    return selectedThing ~= nil and g_ui.isMouseGrabbed()
end

function isUseWithCursorActive()
    return selectedThing ~= nil and selectedType == 'use' and g_ui.isMouseGrabbed()
end

function refreshMouseActionCursor(force)
    if isMouseActionCursorActive() then
        applySelectedCursor(force)
    end
end
...
function onGameStart()
    if g_game.isRecordRestarting() then
...
function save()
    ...
    if gameSelectedPanel then
        settings.selectedContainerPanel = gameSelectedPanel:getId()
    end
...
function load()
    ...
    if settings then
        ...
        if settings.selectedContainerPanel then
            for _, panelData in ipairs(panelsList) do
                if panelData.panel:getId() == settings.selectedContainerPanel then
                    panelsRadioGroup:selectWidget(panelData.checkbox)
                    break
                end
            end
        end
...
function onMouseGrabberMove(self, mousePosition, mouseMoved)
    if isMouseActionCursorActive() then
        applySelectedCursor(true)
    end
end
        ...
        if useThing:isContainer() then
            if useThing:getParentContainer() then
                ...
                menu:addOption(tr('Open in new window'), function()
                    openContainerInNewWindow(useThing)
                end)
            else
                menu:addOption(tr('Open'), function()
    ...
    if creatureThing then
        ...
        if creatureThing:isLocalPlayer() then
            ...
            if creatureThing:isPlayer() then
                ...
                if modules.game_playershop and modules.game_playershop.canOpenStore(creatureThing) then
                    menu:addOption(tr('Open store'), function()
                        modules.game_playershop.requestOpen(creatureThing)
                    end)
                end
...
function openContainerInNewWindow(containerThing)
    if not containerThing or not containerThing:isContainer() then
        return false
    end

    if not containerThing:getParentContainer() then
        return false
    end

    g_game.open(containerThing)
    return true
end
...
function processMouseAction(menuPosition, mouseButton, autoWalkPos, lookThing, useThing, creatureThing, attackCreature)
    ...
    elseif not modules.client_options.getOption('classicControl') then
        ...
        if keyboardModifiers == KeyboardNoModifier and mouseButton == MouseRightButton then
            createThingMenu(menuPosition, lookThing, useThing, creatureThing)
            return true
        elseif useThing and useThing:isContainer() and keyboardModifiers == KeyboardShiftModifier and
            mouseButton == MouseRightButton then
            if openContainerInNewWindow(useThing) then
                return true
            end
        ...
        if lootControlMode == 0 then
            ...
            if mouseButton == MouseRightButton and keyboardModifiers == KeyboardShiftModifier then
                if useThing then
                    if useThing:isContainer() or useThing:isLyingCorpse() then
                        if useThing:isContainer() and openContainerInNewWindow(useThing) then
                            return true
        ...
        if useThing and useThing:isContainer() and keyboardModifiers == KeyboardShiftModifier and
            mouseButton == MouseRightButton then
            if openContainerInNewWindow(useThing) then
                return true
            end
...
function findContentPanelAvailable(child, minContentHeight)
    if gameSelectedPanel and gameSelectedPanel:isVisible() and gameSelectedPanel:isOn() and
        gameSelectedPanel:getWidth() > 0 and gameSelectedPanel:getHeight() > 0 and
        gameSelectedPanel:fits(child, minContentHeight, 0) >= 0 then
        return gameSelectedPanel
    end

    for k, v in pairs(panelsList) do
        if v.panel ~= gameSelectedPanel and v.panel:isVisible() and v.panel:isOn() and v.panel:getWidth() > 0 and
            v.panel:getHeight() > 0 and v.panel:fits(child, minContentHeight, 0) >= 0 then
            return v.panel
        end
    end
...
local function movePanel(mainpanel)
    ...
    for _, widget in pairs(widgets) do
        if widget then
            local minHeight = widget.getMinimumHeight and widget:getMinimumHeight() or widget:getHeight()
            local panel = modules.game_interface.findContentPanelAvailable(widget, minHeight)
            if panel == mainpanel then
                panel = nil
                for _, data in pairs(panelsList) do
                    if data.panel ~= mainpanel and data.panel:isVisible() and data.panel:isOn() and
                        data.panel:getWidth() > 0 and data.panel:getHeight() > 0 and
                        data.panel:fits(widget, minHeight, 0) >= 0 then
                        panel = data.panel
                        break
                    end
                end
            end
            if panel and panel:fits(widget, minHeight, 0) >= 0 then
...
function onDecreaseLeftPanels()
    ...
    if modules.client_options.getOption('showLeftExtraPanel') then
        if not movePanel(gameLeftExtraPanel) then
            return
        end
    ...
    if not g_platform.isMobile() then
        if modules.client_options.getOption('showLeftPanel') then
            if not movePanel(gameLeftPanel) then
                return
            end
...
function onDecreaseRightPanels()
    ...
    if not movePanel(gameRightExtraPanel) then
        rightDecreaseSidePanels:setEnabled(true)
        return
    end
