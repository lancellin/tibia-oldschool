...
local function sanitizeCamFilePart(value)
    local sanitized = tostring(value or ''):gsub('[^%w_%-]', '_')
    sanitized = sanitized:gsub('_+', '_'):sub(1, 48)
    return #sanitized > 0 and sanitized or 'Player'
end

local function createCamFileName(charInfo)
    return string.format('%s_%03d_%s.cam', os.date('%Y%m%d_%H%M%S'),
                         g_clock.millis() % 1000, sanitizeCamFilePart(charInfo.characterName))
end
...
local function tryLogin(charInfo, tries)
    ...
    g_game.loginWorld(G.account, G.password, charInfo.worldName, charInfo.worldHost, charInfo.worldPort,
                      charInfo.characterName, G.authenticatorToken, G.sessionKey, createCamFileName(charInfo))
...
function CharacterList.show()
    ...
    -- onGameEnd also restores the top menu, which can take the root focus
    -- after this function returns. Reapply it once every logout callback has
    -- finished so Enter selects the focused character without a mouse click.
    addEvent(function()
        if charactersWindow and charactersWindow:isVisible() and not g_game.isOnline() then
            charactersWindow:raise()
            charactersWindow:focus()
        end
    end)
