...
local startupLoadingEvent

local function setStartupLoadingProgress(percent, status)
    if not startupLoading then
        return
    end

    startupLoading:recursiveGetChildById('progress'):setPercent(percent)
    startupLoading:recursiveGetChildById('status'):setText(status)
end

local function hideStartupLoading()
    if startupLoading then
        startupLoading:hide()
        startupLoading:lower()
    end
end

local function preloadConfiguredClientAssets(onComplete)
    if not g_settings.getBoolean('preloadAllSprites') then
        return false
    end
    ...
    startupLoading:show()
    startupLoading:raise()
    startupLoading:focus()
    setStartupLoadingProgress(5, tr('Iniciando...'))

    -- Keep the expensive phases disabled while the base client files are opened.
    -- Each phase is scheduled separately so the loading screen is drawn first.
    g_sprites.setPreloadAllSpritesEnabled(false)
    g_things.setPreloadAllTexturesEnabled(false)

    startupLoadingEvent = scheduleEvent(function()
        setStartupLoadingProgress(15, tr('Carregando arquivos do client...'))
        g_game.setClientVersion(clientVersion)
        g_game.setProtocolVersion(g_game.getClientProtocolVersion(clientVersion))

        setStartupLoadingProgress(40, tr('Preparando sprites...'))
        startupLoadingEvent = scheduleEvent(function()
            g_sprites.setPreloadAllSpritesEnabled(true)

            setStartupLoadingProgress(75, tr('Preparando texturas...'))
            startupLoadingEvent = scheduleEvent(function()
                g_things.setPreloadAllTexturesEnabled(true)

                setStartupLoadingProgress(100, tr('Pronto!'))
                startupLoadingEvent = scheduleEvent(function()
                    startupLoadingEvent = nil
                    hideStartupLoading()
                    onComplete()
                end, 150)
            end, 40)
...
function EnterGame.init()
    ...
    startupLoading = g_ui.displayUI('startup_loading')
    startupLoading:hide()
...
function EnterGame.showCamList()
...
local function finishFirstShow()
    EnterGame.show()
...
function EnterGame.firstShow()
    g_window.show()
    if not preloadConfiguredClientAssets(finishFirstShow) then
        finishFirstShow()
    end
end
...
function EnterGame.terminate()
    ...
    if startupLoadingEvent then
        removeEvent(startupLoadingEvent)
        startupLoadingEvent = nil
    end
...
function EnterGame.show()
    ...
    enterGame:raise()
    enterGame:focus()
    -- The native window and the top menu may finish showing after this call
    -- and steal the root focus. Restore it on the next event cycle so Enter
    -- works immediately on startup and after returning to the login screen.
    addEvent(function()
        if enterGame and enterGame:isVisible() and not g_game.isOnline() then
            enterGame:raise()
            enterGame:focus()
        end
    end)
