...
local function syncHdSpriteAntialiasingState()
    local graphicsPanel = panels.graphicsPanel
    if not graphicsPanel or not options.enableHdSprites or not options.enableHdFastPack or not options.antialiasingMode then
        return
    end

    local antialiasingModeCombobox = graphicsPanel:recursiveGetChildById('antialiasingMode')
    local hdSpritesEnabled = options.enableHdSprites.value == true or options.enableHdFastPack.value == true

    if hdSpritesEnabled and options.antialiasingMode.value ~= 2 then
        setOption('antialiasingMode', 2, true)
    end

    if antialiasingModeCombobox then
        antialiasingModeCombobox:setEnabled(not hdSpritesEnabled)
    end
end

local function syncForcedEffectsState()
    for key, forcedValue in pairs(forcedOptionValues) do
        if options[key] and options[key].value ~= forcedValue then
            setOption(key, forcedValue, true)
        end
    end

    local graphicsEffectsPanel = panels.graphicsEffectsPanel
    if not graphicsEffectsPanel then
        return
    end

    for _, widgetId in ipairs({ 'enableLights', 'ambientLight', 'shadowFloorIntensity', 'floorFading', 'floorViewMode' }) do
        local widget = graphicsEffectsPanel:recursiveGetChildById(widgetId)
        if widget then
...
function setOption(key, value, force)
    ...
    if forcedOptionValues[key] ~= nil then
        value = forcedOptionValues[key]
    end
    ...
    if forcedOptionValues[key] ~= nil then
        syncForcedEffectsState()
    end

    if key == 'enableHdSprites' or key == 'enableHdFastPack' or key == 'antialiasingMode' then
        syncHdSpriteAntialiasingState()
    end
end

function syncLockedGraphicsState()
    syncHdSpriteAntialiasingState()
    syncForcedEffectsState()
end
