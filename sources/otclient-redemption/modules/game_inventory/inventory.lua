...
local characterBonusesWindow = nil
local characterBonusesButton = nil
local characterBonuses = {}
...
local function updateCharacterBonuses(charmData)
    characterBonuses = charmData and charmData.characterBonuses or {}
    if not characterBonusesWindow then
        return
    end

    local list = characterBonusesWindow:recursiveGetChildById('characterBonusList')
    list:destroyChildren()

    if #characterBonuses == 0 then
        local row = g_ui.createWidget('CharacterBonusRow', list)
        row.bonusName:setText(tr('No active bonuses'))
        row.bonusValue:setText('')
        return
    end

    for _, bonus in ipairs(characterBonuses) do
        local row = g_ui.createWidget('CharacterBonusRow', list)
        row.bonusName:setText(bonus.name)
        row.bonusName:setTooltip(bonus.source)
        row.bonusValue:setText(bonus.value)
        row.bonusValue:setTooltip(bonus.source)

        local numericValue = tonumber(tostring(bonus.value):match("(-?%d+%.?%d*)"))
        if bonus.name == 'Critical Chance' and numericValue and numericValue > 0 then
            row.bonusValue:setColor('#44AD25')
        else
            row.bonusValue:setColor('#C0C0C0')
        end
    end
end

local function toggleCharacterBonuses()
    if not characterBonusesWindow then
        return
    end

    if characterBonusesButton and characterBonusesButton:isOn() then
        characterBonusesWindow:close()
        characterBonusesButton:setOn(false)
        return
    end

    if not characterBonusesWindow:getParent() then
        local panel = modules.game_interface.findContentPanelAvailable(characterBonusesWindow, characterBonusesWindow:getMinimumHeight())
        if not panel then
            return
        end

        panel:addChild(characterBonusesWindow)
    end

    updateCharacterBonuses()
    characterBonusesWindow:open()
    if characterBonusesButton then
        characterBonusesButton:setOn(true)
    end
end
...
function inventoryController:onInit()
    refreshInventory_panel()
    characterBonusesButton = modules.game_mainpanel.addToggleButton('characterBonusesButton', tr('Character Bonuses'),
        '/images/options/button_skills', toggleCharacterBonuses, false, 4)
    characterBonusesWindow = g_ui.loadUI('characterbonuses')
    characterBonusesWindow:setContentMinimumHeight(44)
    characterBonusesWindow:setup()
    updateCharacterBonuses()
...
function inventoryController:onGameStart()
    ...
    inventoryController:registerEvents(g_game, {
        ...
        onUpdateBestiaryCharmsData = updateCharacterBonuses
    }):execute()

    inventoryShrink = g_settings.getBoolean('mainpanel_shrink_inventory')
    refreshInventorySizes()
    refreshInventory_panel()
    inventoryController.ui:setupOnStart()
    g_game.requestBestiary()
    characterBonusesWindow:setupOnStart()
...
function inventoryController:onGameEnd()
    ...
    characterBonuses = {}
    updateCharacterBonuses()
    if characterBonusesWindow then
        characterBonusesWindow:setParent(nil, true)
    end
    if characterBonusesButton then
        characterBonusesButton:setOn(false)
    end
...
function inventoryController:onTerminate()
    if characterBonusesWindow then
        characterBonusesWindow:destroy()
        characterBonusesWindow = nil
    end
    if characterBonusesButton then
        characterBonusesButton:destroy()
        characterBonusesButton = nil
    end
    if iconTopMenu then
        iconTopMenu:destroy()
        iconTopMenu = nil
...
function extendedView(extendedView)
    if extendedView then
        ...
        if not inventoryController.ui:getParent() then
            rightPanel:insertChild(3, inventoryController.ui)
        end
...
function toggleCharacterBonusesWindow()
    toggleCharacterBonuses()
end

function onCharacterBonusesWindowOpen()
    if characterBonusesButton then
        characterBonusesButton:setOn(true)
    end
end

function onCharacterBonusesWindowClose()
    if characterBonusesButton then
        characterBonusesButton:setOn(false)
    end
end
