...
local function getNpcSpeaker(name, creaturePos)
    if not name or name == '' or name == g_game.getCharacterName() or not creaturePos then
        return nil
    end

    local spectators = g_map.getSpectators(creaturePos, false) or {}
    for _, creature in ipairs(spectators) do
        if creature and creature:isNpc() and creature:getName() == name then
            local npcPos = creature:getPosition()
            if npcPos and positionsMatch(npcPos, creaturePos) then
                return creature
            end
        end
    end
...
local function isLegacyNpcChatClient()
    return g_game.getClientVersion() > 0 and g_game.getClientVersion() < 840
end

local function isNpcSpeakerMessage(name, mode, creaturePos)
    if mode == MessageModes.NpcFromStartBlock or mode == MessageModes.NpcFrom then
        return true
    end
    ...
    return getNpcSpeaker(name, creaturePos) ~= nil
end

function sendNpcMessage(text)
    if not text or text == '' then
        return false
    end

    if isLegacyNpcChatClient() then
...
function addTabText(text, speaktype, tab, creatureName)
    ...
    local isNpcMessage = speaktype.npcChat
    if isNpcMessage and (g_game.getCharacterName() ~= creatureName or g_game.getCharacterName() == 'Account Manager') then
...
function sendCurrentMessage()
    ...
    -- send message
    sendMessage(message)
    if modules.client_options.getOption('returnDisablesChat') and isChatEnabled() and not consoleToggleChat.isChecked then
        toggleChat()
    end
end
...
function sendMessage(message, tab)
    ...
    if (channel ~= nil or tab == defaultTab or isLootChannelTab) and not chatCommandPrivateReady then
        ...
        elseif tab.npcChat then
            if isLegacyNpcChatClient() then
                g_game.talkChannel(MessageModes.Say, 0, message)
                return
            end
...
function onTalk(name, level, mode, message, channelId, creaturePos)
    ...
    local isNpcMode = isNpcSpeakerMessage(name, mode, creaturePos)
    ...
    if isNpcMode and not speaktype.npcChat then
        speaktype = table.copy(speaktype)
        speaktype.npcChat = true
    end
