...
function UIMiniWindowContainer:scheduleInsert(widget, index)
    for scheduledIndex, scheduledWidget in pairs(self.scheduledWidgets) do
        if scheduledWidget == widget and scheduledIndex ~= index then
            ...
            while true do
                ...
                for nIndex, nWidget in pairs(self.scheduledWidgets) do
                    if nIndex - 1 <= self:getChildCount() then
                        local scheduledParent = nWidget:getParent()
                        if scheduledParent and scheduledParent ~= self then
                            scheduledParent:removeChild(nWidget)
                        elseif scheduledParent == self and self:getChildIndex(nWidget) > 0 then
