...
function UIResizeBorder:onDestroy()
    if self.hovering then
        ...
        if modules.game_interface and modules.game_interface.isMouseActionCursorActive and
            modules.game_interface.isMouseActionCursorActive() then
            if not (modules.client_options and modules.client_options.getOption('nativeCursor')) then
                g_mouse.popCursor(self.cursortype)
            end
            modules.game_interface.refreshMouseActionCursor(true)
        elseif modules.client_options and modules.client_options.getOption('nativeCursor') then
...
function UIResizeBorder:onHoverChange(hovered)
    local actionCursorActive = modules.game_interface and modules.game_interface.isMouseActionCursorActive and
        modules.game_interface.isMouseActionCursorActive()
    if actionCursorActive then
        if not hovered then
