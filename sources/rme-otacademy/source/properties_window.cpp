...
wxWindow* PropertiesWindow::createGeneralPanel(wxWindow* parent)
{
	...
	actionIdField = newd wxSpinCtrl(panel, wxID_ANY, i2ws(edit_item->getActionID()), wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS, 0, 0xFFFF, edit_item->getActionID());
	gridsizer->Add(actionIdField, wxSizerFlags(1).Expand());

	gridsizer->Add(newd wxStaticText(panel, wxID_ANY, "Unique ID"));
	uniqueIdField = newd wxSpinCtrl(panel, wxID_ANY, i2ws(edit_item->getUniqueID()), wxDefaultPosition, wxSize(-1, 20), wxSP_ARROW_KEYS, 0, 0xFFFF, edit_item->getUniqueID());
	gridsizer->Add(uniqueIdField, wxSizerFlags(1).Expand());

	if(Teleport* teleport = dynamic_cast<Teleport*>(edit_item)) {
		gridsizer->Add(newd wxStaticText(panel, wxID_ANY, "Destination"));

		wxSizer* destinationSizer = newd wxBoxSizer(wxHORIZONTAL);
		teleportDestinationXField = newd wxSpinCtrl(panel, wxID_ANY, i2ws(teleport->getX()), wxDefaultPosition, wxSize(-1, 20), wxSP_ARROW_KEYS, 0, edit_map->getWidth(), teleport->getX());
		destinationSizer->Add(teleportDestinationXField, wxSizerFlags(3).Expand());
		teleportDestinationYField = newd wxSpinCtrl(panel, wxID_ANY, i2ws(teleport->getY()), wxDefaultPosition, wxSize(-1, 20), wxSP_ARROW_KEYS, 0, edit_map->getHeight(), teleport->getY());
		destinationSizer->Add(teleportDestinationYField, wxSizerFlags(3).Expand());
		teleportDestinationZField = newd wxSpinCtrl(panel, wxID_ANY, i2ws(teleport->getZ()), wxDefaultPosition, wxSize(-1, 20), wxSP_ARROW_KEYS, 0, MAP_MAX_LAYER, teleport->getZ());
		destinationSizer->Add(teleportDestinationZField, wxSizerFlags(2).Expand());
...
{
	if(actionIdField) {
		edit_item->setActionID(actionIdField->GetValue());
	}

	if(uniqueIdField) {
		edit_item->setUniqueID(uniqueIdField->GetValue());
	}

	Teleport* teleport = dynamic_cast<Teleport*>(edit_item);
	if(teleport && teleportDestinationXField && teleportDestinationYField && teleportDestinationZField) {
		teleport->setDestination(Position(
			teleportDestinationXField->GetValue(),
			teleportDestinationYField->GetValue(),
			teleportDestinationZField->GetValue()
		));
	}
}
...
{
	if(actionIdField) {
		int actionId = actionIdField->GetValue();
		if((actionId < 100 || actionId > 0xFFFF) && actionId != 0) {
	...
	if(uniqueIdField) {
		...
		if((uniqueId < 1000 || uniqueId > 0xFFFF) && uniqueId != 0) {
			g_gui.PopupDialog(this, "Error", "Unique ID must be between 1000 and 65535.", wxOK);
			return;
		}
	}

	if(dynamic_cast<Teleport*>(edit_item) && teleportDestinationXField && teleportDestinationYField && teleportDestinationZField) {
		Position destination(
			teleportDestinationXField->GetValue(),
			teleportDestinationYField->GetValue(),
			teleportDestinationZField->GetValue()
		);
		if(edit_map->getTile(destination) == nullptr || edit_map->getTile(destination)->isBlocking()) {
