...
namespace {
	bool isValidFloorPersistenceInstanceId(const std::string& instanceId)
	{
		if (instanceId.size() != 32) {
			return false;
		}

		for (char character : instanceId) {
			const bool isNumber = character >= '0' && character <= '9';
			const bool isLowerHex = character >= 'a' && character <= 'f';
			if (!isNumber && !isLowerHex) {
				return false;
			}
		}
		return true;
	...
	std::string generateFloorPersistenceInstanceId()
	{
...
Item* Item::clone() const
{
	...
	if (attributes) {
		...
		if (item->removeCustomAttribute(ITEM_CUSTOM_ATTRIBUTE_FLOOR_INSTANCE_ID)) {
			std::string cloneInstanceId;
			item->ensureFloorPersistenceInstanceId(cloneInstanceId);
		}
		if (item->getDuration() > 0) {
			if (g_game.isEmergencyActive()) {
...
bool Item::isFloorPersistenceInstanceEligible() const
{
	return !isFloorPersistenceCreatureCorpse() && !isLoadedFromMap() && isMoveable() &&
	       !isStackable() && !hasAttribute(ITEM_ATTRIBUTE_UNIQUEID);
}

bool Item::isFloorPersistenceIdentityReady() const
{
	if (isFloorPersistenceCreatureCorpse()) {
		// Creature corpses and everything that remains inside them are
		// deliberately outside floor persistence.
		return true;
	}
	...
	if (!isMoveable() || isStackable() || hasAttribute(ITEM_ATTRIBUTE_UNIQUEID)) {
		return true;
	}

	return !getFloorPersistenceInstanceId().empty();
}

bool Item::isFloorPersistenceSubtreeReady() const
{
	if (!isFloorPersistenceIdentityReady()) {
		return false;
	}

	const Container* container = getContainer();
	return !container || container->hasFloorPersistenceIdentifiedSubtree();
}

bool Item::isFloorPersistenceCreatureCorpse() const
{
	// The dedicated marker survives every decay stage. loot_monster_name keeps
	// snapshots produced by older binaries recognizable during migration.
	return getCustomAttribute(ITEM_CUSTOM_ATTRIBUTE_FLOOR_CREATURE_CORPSE) != nullptr ||
	       getCustomAttribute(ITEM_CUSTOM_ATTRIBUTE_LOOT_MONSTER_NAME) != nullptr;
}

std::string Item::getFloorPersistenceInstanceId() const
{
	if (isFloorPersistenceCreatureCorpse()) {
		return {};
	}

	const ItemAttributes::CustomAttribute* attribute = getCustomAttribute(ITEM_CUSTOM_ATTRIBUTE_FLOOR_INSTANCE_ID);
	if (!attribute) {
		return {};
	}

	const std::string* instanceId = boost::get<std::string>(&attribute->value);
	if (!instanceId || !isValidFloorPersistenceInstanceId(*instanceId)) {
		return {};
	}
	return *instanceId;
}

uint32_t Item::getFloorPersistenceLastActorGuid() const
{
	if (attributes && attributes->hasAttribute(ITEM_ATTRIBUTE_LASTACTORGUID)) {
		const int64_t playerGuid = attributes->getIntAttr(ITEM_ATTRIBUTE_LASTACTORGUID);
		if (playerGuid > 0 && playerGuid <= std::numeric_limits<uint32_t>::max()) {
			return static_cast<uint32_t>(playerGuid);
		}
	}

	// Compatibility with snapshots and player blobs produced before the compact
	// native attribute existed. The next successful attribution migrates it.
	const ItemAttributes::CustomAttribute* attribute =
		getCustomAttribute(ITEM_CUSTOM_ATTRIBUTE_FLOOR_LAST_ACTOR_GUID);
	if (!attribute) {
		return 0;
	}

	const int64_t* playerGuid = boost::get<int64_t>(&attribute->value);
	if (!playerGuid || *playerGuid <= 0 || *playerGuid > std::numeric_limits<uint32_t>::max()) {
		return 0;
	}
	return static_cast<uint32_t>(*playerGuid);
}

bool Item::setFloorPersistenceLastActorGuid(uint32_t playerGuid)
{
	if (playerGuid == 0) {
		return false;
	}

	const bool alreadyNative = attributes &&
		attributes->hasAttribute(ITEM_ATTRIBUTE_LASTACTORGUID);
	const bool changed = getFloorPersistenceLastActorGuid() != playerGuid || !alreadyNative ||
		getCustomAttribute(ITEM_CUSTOM_ATTRIBUTE_FLOOR_LAST_ACTOR_GUID) != nullptr;
	if (!changed) {
		return false;
	}

	getAttributes()->setIntAttr(ITEM_ATTRIBUTE_LASTACTORGUID, playerGuid);
	removeCustomAttribute(ITEM_CUSTOM_ATTRIBUTE_FLOOR_LAST_ACTOR_GUID);
	return true;
}

bool Item::hasFloorPersistenceLastActorConsistentSubtree() const
{
	return attributes &&
		attributes->getIntAttr(ITEM_ATTRIBUTE_LASTACTORCONSISTENT) == 1;
}

void Item::setFloorPersistenceLastActorConsistentSubtree(bool consistent)
{
	if (consistent) {
		getAttributes()->setIntAttr(ITEM_ATTRIBUTE_LASTACTORCONSISTENT, 1);
	} else if (attributes) {
		attributes->removeAttribute(ITEM_ATTRIBUTE_LASTACTORCONSISTENT);
	}
}

bool Item::ensureFloorPersistenceInstanceId(std::string& instanceId)
{
	instanceId = getFloorPersistenceInstanceId();
	if (!instanceId.empty()) {
		return true;
	}

	if (!isFloorPersistenceInstanceEligible() || getCustomAttribute(ITEM_CUSTOM_ATTRIBUTE_FLOOR_INSTANCE_ID)) {
		return false;
	}

	instanceId = generateFloorPersistenceInstanceId();
	std::string key = ITEM_CUSTOM_ATTRIBUTE_FLOOR_INSTANCE_ID;
	setCustomAttribute(key, instanceId);
	markFloorPersistenceAttributeDirty(true);
	return true;
}

void Item::markAsPlayerMovedForFloorPersistence()
{
	auto identifyItem = [](Item* item) {
		item->removeCustomAttribute(ITEM_CUSTOM_ATTRIBUTE_FLOOR_DEATH_BUNDLE);
		item->setLoadedFromMap(false);

		if (item->isFloorPersistenceCreatureCorpse()) {
			// A legacy corpse may already carry an identity. Remove it without
			// descending into the disposable corpse subtree.
			item->removeCustomAttribute(ITEM_CUSTOM_ATTRIBUTE_FLOOR_INSTANCE_ID);
			return true;
		}

		std::string instanceId;
		item->ensureFloorPersistenceInstanceId(instanceId);
		return item->isFloorPersistenceIdentityReady();
	};

	bool subtreeReady = identifyItem(this);
	...
	if (isFloorPersistenceCreatureCorpse()) {
		container->setFloorPersistenceIdentifiedSubtree(true);
		return;
	}

	if (subtreeReady && container->hasFloorPersistenceIdentifiedSubtree()) {
		return;
	}

	container->setFloorPersistenceIdentifiedSubtree(false);
	std::vector<Container*> visitedContainers {container};
	std::vector<Container*> pendingContainers {container};
	while (!pendingContainers.empty()) {
		Container* currentContainer = pendingContainers.back();
		pendingContainers.pop_back();
		for (Item* child : currentContainer->getItemList()) {
			subtreeReady = identifyItem(child) && subtreeReady;
			Container* childContainer = child->getContainer();
			if (!childContainer) {
				continue;
			}

			if (child->isFloorPersistenceCreatureCorpse()) {
				childContainer->setFloorPersistenceIdentifiedSubtree(true);
				continue;
			}

			visitedContainers.push_back(childContainer);
			pendingContainers.push_back(childContainer);
		}
	}

	if (subtreeReady) {
		for (Container* visitedContainer : visitedContainers) {
			visitedContainer->setFloorPersistenceIdentifiedSubtree(true);
		}
	}
}

void Item::setLoadedFromMapRecursively(bool value)
{
	setLoadedFromMap(value);
	Container* container = getContainer();
	if (!container) {
		return;
	}

	for (ContainerIterator iterator = container->iterator(); iterator.hasNext(); iterator.advance()) {
		Item* child = *iterator;
		child->setLoadedFromMap(value);
	}
}

void Item::markFloorPersistenceAttributeDirty(bool explicitPlayerAction) const
{
	...
	if (tile) {
		g_game.markFloorTileDirty(*tile, FLOOR_DIRTY_ATTRIBUTE_UPDATE,
			explicitPlayerAction ? FLOOR_DIRTY_ORIGIN_EXPLICIT : FLOOR_DIRTY_ORIGIN_SYSTEM);
	}
}
...
bool Item::equals(const Item* otherItem) const
{
	...
	const uint32_t ignoredAttributeBits =
		static_cast<uint32_t>(ITEM_ATTRIBUTE_CUSTOM) |
		static_cast<uint32_t>(ITEM_ATTRIBUTE_LASTACTORGUID) |
		static_cast<uint32_t>(ITEM_ATTRIBUTE_LASTACTORCONSISTENT);
	const uint32_t attributeBits = attributes ? attributes->attributeBits : 0;
	const uint32_t otherAttributeBits = otherAttributes ? otherAttributes->attributeBits : 0;
	if ((attributeBits & ~ignoredAttributeBits) != (otherAttributeBits & ~ignoredAttributeBits)) {
		return false;
	}
	...
	if (attributes && otherAttributes) {
		...
		for (const auto& attribute : attributeList) {
			...
			if (attribute.type == ITEM_ATTRIBUTE_LASTACTORGUID ||
			    attribute.type == ITEM_ATTRIBUTE_LASTACTORCONSISTENT) {
				continue;
			}
	...
	const ItemAttributes::CustomAttributeMap* customAttributes = attributes ? attributes->getCustomAttributeMap() : nullptr;
	...
	auto countComparableCustomAttributes = [&isComparableCustomAttribute](
			const ItemAttributes::CustomAttributeMap* customAttributeMap) {
		size_t count = 0;
		if (customAttributeMap) {
			for (const auto& entry : *customAttributeMap) {
				if (isComparableCustomAttribute(entry.first)) {
					++count;
				}
			}
		}
	...
	if (countComparableCustomAttributes(customAttributes) != countComparableCustomAttributes(otherCustomAttributes)) {
		return false;
	}

	if (customAttributes) {
		for (const auto& entry : *customAttributes) {
			if (!isComparableCustomAttribute(entry.first)) {
				continue;
			}

			if (!otherCustomAttributes) {
				return false;
			}

			auto otherIterator = otherCustomAttributes->find(entry.first);
			if (otherIterator == otherCustomAttributes->end() || entry.second.value != otherIterator->second.value) {
				return false;
			}
		}
	}
...
Attr_ReadValue Item::readAttr(AttrTypes_t attr, PropStream& propStream)
{
	switch (attr) {
		...
		case ATTR_LAST_ACTOR_GUID: {
			uint32_t playerGuid;
			if (!propStream.read<uint32_t>(playerGuid)) {
				return ATTR_READ_ERROR;
			}
			if (playerGuid != 0) {
				getAttributes()->setIntAttr(ITEM_ATTRIBUTE_LASTACTORGUID, playerGuid);
		...
		case ATTR_LAST_ACTOR_CONSISTENT: {
			uint8_t consistent;
			if (!propStream.read<uint8_t>(consistent)) {
				return ATTR_READ_ERROR;
			}
			if (consistent != 0) {
				getAttributes()->setIntAttr(ITEM_ATTRIBUTE_LASTACTORCONSISTENT, 1);
...
void Item::serializeAttr(PropWriteStream& propWriteStream) const
{
	...
	if (hasAttribute(ITEM_ATTRIBUTE_LASTACTORGUID)) {
		propWriteStream.write<uint8_t>(ATTR_LAST_ACTOR_GUID);
		propWriteStream.write<uint32_t>(
			static_cast<uint32_t>(getIntAttr(ITEM_ATTRIBUTE_LASTACTORGUID)));
	}

	if (hasAttribute(ITEM_ATTRIBUTE_LASTACTORCONSISTENT)) {
		propWriteStream.write<uint8_t>(ATTR_LAST_ACTOR_CONSISTENT);
		propWriteStream.write<uint8_t>(
			getIntAttr(ITEM_ATTRIBUTE_LASTACTORCONSISTENT) == 1 ? 1 : 0);
	}
	...
	if (hasAttribute(ITEM_ATTRIBUTE_CUSTOM)) {
		...
		// Custom attributes live in an unordered_map. A canonical key order keeps
		// shadow snapshot checksums stable across save/load cycles and processes.
		std::vector<const ItemAttributes::CustomAttributeMap::value_type*> sortedAttributes;
		sortedAttributes.reserve(customAttrMap->size());
		for (const auto& entry : *customAttrMap) {
			sortedAttributes.push_back(&entry);
		}
		std::sort(sortedAttributes.begin(), sortedAttributes.end(), [](const auto* left, const auto* right) {
