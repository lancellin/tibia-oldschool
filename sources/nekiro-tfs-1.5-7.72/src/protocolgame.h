...
class ProtocolGame final : public Protocol
{
	...
	private:
		...
		void parseBestiaryRequest(NetworkMessage& msg);
		void parseBestiaryRequestOverview(NetworkMessage& msg);
		void parseBestiaryRequestSearch(NetworkMessage& msg);
		void parseCharmAction(NetworkMessage& msg);
		bool checkClientRequestRateLimit(int64_t& nextAllowedRequest, uint32_t cooldownMs);
		bool checkWalkRequestRateLimit();
		void handleAttackTargetRequest(uint32_t creatureId);
		void flushPendingAttackTarget();
		...
		void sendBestiaryCharmsData();
		void sendBestiaryOverview(const std::string& categoryName, bool search, const std::vector<uint16_t>& creatureIds);
		void sendBestiaryMonsterData(uint16_t creatureId);
		...
		int64_t nextBestiaryRequest = 0;
		int64_t nextBestiaryOverviewRequest = 0;
		int64_t nextBestiarySearchRequest = 0;
