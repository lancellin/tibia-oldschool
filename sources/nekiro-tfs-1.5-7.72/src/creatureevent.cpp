...
namespace {
	constexpr uint8_t ADVANCE_EXTENDED_OPCODE = 10;
}
...
bool CreatureEvents::playerAdvance(Player* player, skills_t skill, uint32_t oldLevel,
                                       uint32_t newLevel)
{
	if (player && newLevel > oldLevel && skill <= SKILL_LEVEL &&
		player->getOperatingSystem() >= CLIENTOS_OTCLIENT_LINUX) {
		player->sendExtendedOpcode(ADVANCE_EXTENDED_OPCODE,
			std::to_string(static_cast<uint8_t>(skill)) + "|" + std::to_string(newLevel));
	}
