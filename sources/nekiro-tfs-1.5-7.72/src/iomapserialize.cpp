...
bool IOMapSerialize::saveHouseInfo()
{
	...
	for (const auto& it : g_game.map.houses.getHouses()) {
		...
		if (house->getAccessList(GUEST_LIST, listText) && !listText.empty()) {
			if (!stmt.addRow(fmt::format("{:d}, {}, {:s}", house->getId(), static_cast<uint32_t>(GUEST_LIST), db.escapeString(listText)))) {
				return false;
			}
		...
		if (house->getAccessList(SUBOWNER_LIST, listText) && !listText.empty()) {
			if (!stmt.addRow(fmt::format("{:d}, {}, {:s}", house->getId(), static_cast<uint32_t>(SUBOWNER_LIST), db.escapeString(listText)))) {
				return false;
			}
...
bool IOMapSerialize::saveHouseData(House* house)
{
	if (!house) {
		return false;
	}
