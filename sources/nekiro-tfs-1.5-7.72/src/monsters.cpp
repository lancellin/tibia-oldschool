...
bool Monsters::deserializeSpell(const pugi::xml_node& node, spellBlock_t& sb, const std::string& description)
{
	...
	if (isScripted) {
		...
		const bool customParalyze = tmpName.rfind("paralyze_", 0) == 0;
		const bool customHaste = tmpName.rfind("haste_", 0) == 0;
		const bool customSkills = tmpName.rfind("skills_", 0) == 0;
		const bool customDjinnElectrify = (tmpName == "djinn electrify" || tmpName == "djinn electrify area");
		if (customParalyze || customHaste) {
			tmpName = "speed";
		} else if (customSkills) {
			tmpName = "strength";
		} else if (customDjinnElectrify) {
			tmpName = "energy";
		}
		...
		if (tmpName == "melee") {
			...
			if (customParalyze && minSpeedChange == 0 && maxSpeedChange == 0) {
				minSpeedChange = -700;
				maxSpeedChange = -700;
				if (!node.attribute("duration")) {
					duration = 10000;
				}
			} else if (customHaste && minSpeedChange == 0 && maxSpeedChange == 0) {
				minSpeedChange = 250;
				maxSpeedChange = 250;
				if (!node.attribute("duration")) {
					duration = 5000;
