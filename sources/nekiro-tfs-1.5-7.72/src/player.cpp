...
void removeOpenContainerRestoreAttr(Item* item)
{
	...
	std::string key = OPEN_CONTAINER_RESTORE_ATTR;
	item->removeCustomAttribute(key);

	if (Container* container = item->getContainer()) {
		for (Item* child : container->getItemList()) {
			removeOpenContainerRestoreAttr(child);
		}
	}
}

void collectMarkedOpenContainers(Item* item, std::vector<std::pair<uint8_t, Container*>>& markedContainers)
{
	if (!item) {
		return;
	}
	...
	if (Container* container = item->getContainer()) {
		if (const ItemAttributes::CustomAttribute* attr = item->getCustomAttribute(OPEN_CONTAINER_RESTORE_ATTR)) {
			const int64_t value = attr->value.type() == typeid(int64_t) ? boost::get<int64_t>(attr->value) : 0;
			const int64_t version = value >> 8;
			const int64_t cid = (value & 0xFF) - 1;
			if (version == OPEN_CONTAINER_RESTORE_ATTR_VERSION && cid >= 0 && cid <= 0xF) {
				markedContainers.emplace_back(static_cast<uint8_t>(cid), container);
			}
		}

		for (Item* child : container->getItemList()) {
			collectMarkedOpenContainers(child, markedContainers);
		}
	}
}
}
...
const std::vector<CharmDefinition>& getCharmDefinitions()
{
	static const std::vector<CharmDefinition> definitions = {
		{CHARM_SAVAGE_BLOW, "Savage Blow", "Has a chance to turn a direct attack into a devastating critical strike.\n\nWorks with melee attacks, ranged attacks, runes and spells.\n\nPvP Adjustment:\nOnly 50% of the extra critical damage is applied against players.\n\nStage I\nCritical Chance: 4%\nPvE Damage: Maximum hit + 5-25%\nPvP Damage: Maximum hit + 2.5-12.5%\n\nStage II\nCritical Chance: 6%\nPvE Damage: Maximum hit + 8-30%\nPvP Damage: Maximum hit + 4-15%\n\nStage III\nCritical Chance: 8%\nPvE Damage: Maximum hit + 15-40%\nPvP Damage: Maximum hit + 7.5-20%\n\nLegends tell of warriors who could see weaknesses invisible to others. With a single perfectly timed strike, they would shatter armor, break morale and bring down foes far stronger than themselves. Such attacks became known as Savage Blows.", 240, 19, CharmEffectType::CRITICAL, 4, 0, 5, 25, -1},
		{CHARM_VAMPIRIC_EMBRACE, "Vampiric Embrace", "Each creature damaged has a 20% chance to restore health equal to 6% of the damage dealt.\n\nWorks with melee attacks, ranged attacks, runes and spells, including area attacks.\n\nDoes not trigger against players. Damage over time and condition ticks do not trigger this charm.\n\nAncient blood rites teach that every wound carries a fragment of life. Those who master the embrace draw that fading strength into themselves before it is lost.", 240, 17, CharmEffectType::LIFE_LEECH, 20, 6, 0, 0, 0},
		{CHARM_VOIDS_CALL, "Void's Call", "Each creature damaged has a 10% chance to restore mana equal to 4% of the damage dealt.\n\nArea attacks retain 60% effectiveness, restoring 2.4% of the damage dealt by successful targets.\n\nDoes not trigger against players or summons. Damage over time and condition ticks do not trigger this charm.\n\nThe void answers no prayer, yet those who hear its call learn to reclaim a trace of the power consumed by every spell.", 240, 18, CharmEffectType::MANA_LEECH, 10, 4, 0, 0, 40},
		{CHARM_DODGE, "Dodge", "Has a 3% chance to completely avoid a direct damage hit.\n\nWorks against monsters, players and summons. Melee attacks, ranged attacks, runes and direct-damage spells can be dodged.\n\nDamage over time, fields, poison, burning and other condition ticks do not trigger this charm.\n\nA seasoned fighter survives not by enduring every blow, but by being elsewhere when the fatal strike arrives.", 240, 8, CharmEffectType::DODGE, 3, 0, 0, 0, -1},
	};
	return definitions;
}

const CharmDefinition* getCharmDefinition(uint8_t charmId)
{
	const auto& definitions = getCharmDefinitions();
	const auto it = std::find_if(definitions.begin(), definitions.end(), [charmId](const CharmDefinition& definition) {
		return definition.id == charmId;
	});
...
namespace
{
	bool isShieldDefenseItem(const Item* item)
	{
		if (!item || item->getDefense() <= 0) {
			return false;
		}

		return item->getWeaponType() == WEAPON_SHIELD || item->getWeaponType() == WEAPON_NONE;
	}

	bool isCustomDefenseVocation(const Player* player)
	{
		switch (player->getVocation()->getId()) {
			case 1:
			case 2:
			case 4:
			case 5:
	...
	double getCustomDefenseFactor(const Player* player)
	{
		switch (player->getVocation()->getId()) {
			case 4:
			case 8:
	...
	int32_t getCustomArmorMinBlock(int32_t armorValue)
	{
		if (armorValue <= 0) {
			return 0;
		}

		return static_cast<int32_t>(std::floor(std::pow(armorValue, 4.0 / 3.0) * 0.24));
	}

	int32_t getCustomArmorMaxBlock(int32_t armorValue)
	{
		if (armorValue <= 0) {
			return 0;
		}

		return static_cast<int32_t>(std::floor(std::pow(armorValue, 3.0 / 2.0) * 0.22));
	}
}

PlayerCharmState Player::getCharmState(uint8_t charmId) const
{
	const auto it = charmStates.find(charmId);
	if (it == charmStates.end()) {
		return PlayerCharmState::LOCKED;
	}
	return it->second;
}
...
bool Player::loadCharmStatesFromDatabase()
{
	...
	do {
		const uint8_t charmId = static_cast<uint8_t>(result->getNumber<uint16_t>("charm_id"));
		const uint8_t stateValue = static_cast<uint8_t>(result->getNumber<uint16_t>("state"));
		if (!getCharmDefinition(charmId) || stateValue > static_cast<uint8_t>(PlayerCharmState::ACTIVE)) {
			continue;
		}

		charmStates[charmId] = static_cast<PlayerCharmState>(stateValue);
	} while (result->next());
...
bool Player::setCharmState(uint8_t charmId, PlayerCharmState state, bool persist)
{
	if (!getCharmDefinition(charmId)) {
		return false;
	}

	const PlayerCharmState previousState = getCharmState(charmId);
	if (state == PlayerCharmState::LOCKED) {
		charmStates.erase(charmId);
	} else {
		charmStates[charmId] = state;
	}
	...
	if (persist) {
		...
		if (state == PlayerCharmState::LOCKED) {
			persisted = db.executeQuery(fmt::format(
				"DELETE FROM `player_charms` WHERE `player_id` = {:d} AND `charm_id` = {:d}",
				getGUID(), charmId));
		} else {
			persisted = db.executeQuery(fmt::format(
				"INSERT INTO `player_charms` (`player_id`, `charm_id`, `state`, `updated_at`) "
				"VALUES ({:d}, {:d}, {:d}, {:d}) ON DUPLICATE KEY UPDATE `state` = VALUES(`state`), `updated_at` = VALUES(`updated_at`)",
				getGUID(), charmId, static_cast<uint8_t>(state), OTSYS_TIME()));
		}

		if (!persisted) {
			if (previousState == PlayerCharmState::LOCKED) {
				charmStates.erase(charmId);
			} else {
				charmStates[charmId] = previousState;
			}
			recalculateCharacterBonuses();
			return false;
		}
...
bool Player::unlockCharm(uint8_t charmId)
{
	...
	return setCharmState(charmId, PlayerCharmState::UNLOCKED);
}
...
bool Player::activateCharm(uint8_t charmId)
{
	...
	return setCharmState(charmId, PlayerCharmState::ACTIVE);
}

uint32_t Player::getSpentCharmPoints() const
{
	uint32_t spentPoints = 0;
	for (const CharmDefinition& definition : getCharmDefinitions()) {
		if (getCharmState(definition.id) != PlayerCharmState::LOCKED) {
			spentPoints += definition.unlockCost;
		}
	}
	return spentPoints;
}

void Player::recalculateCharacterBonuses()
{
	CharacterBonuses bonuses;
	for (int32_t skill = SKILL_FIRST; skill <= SKILL_LAST; ++skill) {
		bonuses.skillBonus[skill] = varSkills[skill];
	}

	bonuses.equipmentCriticalChance = getSpecialSkill(SPECIALSKILL_CRITICALHITCHANCE);
	bonuses.equipmentCriticalDamagePercent = getSpecialSkill(SPECIALSKILL_CRITICALHITAMOUNT);
	bonuses.equipmentLifeLeechChance = getSpecialSkill(SPECIALSKILL_LIFELEECHCHANCE);
	bonuses.equipmentLifeLeechAmount = getSpecialSkill(SPECIALSKILL_LIFELEECHAMOUNT);
	bonuses.equipmentManaLeechChance = getSpecialSkill(SPECIALSKILL_MANALEECHCHANCE);
	bonuses.equipmentManaLeechAmount = getSpecialSkill(SPECIALSKILL_MANALEECHAMOUNT);

	const CharmDefinition* criticalCharm = getCharmDefinition(CHARM_SAVAGE_BLOW);
	if (criticalCharm && getCharmState(criticalCharm->id) == PlayerCharmState::ACTIVE) {
		bonuses.charmCriticalChance = criticalCharm->chance;
		bonuses.charmCriticalDamageMinPercent = criticalCharm->criticalDamageMinPercent;
		bonuses.charmCriticalDamageMaxPercent = criticalCharm->criticalDamageMaxPercent;
	}

	const CharmDefinition* lifeLeechCharm = getCharmDefinition(CHARM_VAMPIRIC_EMBRACE);
	if (lifeLeechCharm && getCharmState(lifeLeechCharm->id) == PlayerCharmState::ACTIVE) {
		bonuses.charmLifeLeechChance = lifeLeechCharm->chance;
		bonuses.charmLifeLeechAmount = lifeLeechCharm->amountPercent;
	}

	const CharmDefinition* manaLeechCharm = getCharmDefinition(CHARM_VOIDS_CALL);
	if (manaLeechCharm && getCharmState(manaLeechCharm->id) == PlayerCharmState::ACTIVE) {
		bonuses.charmManaLeechChance = manaLeechCharm->chance;
		bonuses.charmManaLeechAmount = manaLeechCharm->amountPercent;
		bonuses.charmManaLeechAreaEfficiency = 100 - manaLeechCharm->aoeReductionPercent;
	}

	const CharmDefinition* dodgeCharm = getCharmDefinition(CHARM_DODGE);
	if (dodgeCharm && getCharmState(dodgeCharm->id) == PlayerCharmState::ACTIVE) {
		bonuses.charmDodgeChance = dodgeCharm->chance;
	}

	bonuses.totalCriticalChance = bonuses.equipmentCriticalChance + bonuses.charmCriticalChance;

	characterBonuses = bonuses;
}

void Player::sendCharmData() const
{
	if (client) {
		client->sendBestiaryCharmsData();
	}
}
...
Player::~Player()
{
	if (playerIOReservationId != 0) {
		g_playerIOManager.releaseLegacyOperation(playerIOReservationId);
		playerIOReservationId = 0;
	}
...
int32_t Player::getDefenseReduction(Creature* attacker, CombatType_t) const
{
	if (fightMode == FIGHTMODE_DEFENSE && attacker && !attacker->getPlayer() && isCustomDefenseVocation(this)) {
		int32_t defenseSkill = getSkillLevel(SKILL_FIST);
		...
		const double maxBlock = std::floor((defenseValue * ((defenseSkill * defenseSkill) / 800.0) * getCustomDefenseFactor(this)) + 10.0);
		return uniform_random(0, std::max<int32_t>(0, static_cast<int32_t>(maxBlock)));
	}
...
int32_t Player::getArmorReduction(Creature*, CombatType_t) const
{
	const int32_t armorValue = getArmor();
	const int32_t minBlock = getCustomArmorMinBlock(armorValue);
	const int32_t maxBlock = getCustomArmorMaxBlock(armorValue);
	if (maxBlock <= 0) {
		return 0;
	}

	return uniform_random(std::min(minBlock, maxBlock), maxBlock);
}
...
void Player::saveOpenContainerState()
{
	for (int32_t slotId = CONST_SLOT_FIRST; slotId <= CONST_SLOT_LAST; ++slotId) {
		removeOpenContainerRestoreAttr(inventory[slotId]);
	}

	for (const auto& it : depotLockerMap) {
		removeOpenContainerRestoreAttr(it.second.get());
	}

	for (const auto& it : depotChests) {
		removeOpenContainerRestoreAttr(it.second);
	}

	for (const auto& it : openContainers) {
		const uint8_t cid = it.first;
		Container* container = it.second.container;
		if (!container || cid > 0xF || container->getTopParent() != this) {
			continue;
		}

		std::string key = OPEN_CONTAINER_RESTORE_ATTR;
		const int64_t value = (OPEN_CONTAINER_RESTORE_ATTR_VERSION << 8) | (static_cast<int64_t>(cid) + 1);
		container->setCustomAttribute(key, value);
	}

}

void Player::restoreOpenContainerState()
{
	if (!openContainers.empty()) {
		for (const auto& it : openContainers) {
			Container* container = it.second.container;
			if (!container || container->getTopParent() != this) {
				continue;
			}

			sendContainer(it.first, container, container->hasParent(), it.second.index);
		}

		return;
	}

	std::vector<std::pair<uint8_t, Container*>> markedContainers;
	markedContainers.reserve(16);

	for (int32_t slotId = CONST_SLOT_FIRST; slotId <= CONST_SLOT_LAST; ++slotId) {
		collectMarkedOpenContainers(inventory[slotId], markedContainers);
	}

	if (markedContainers.empty()) {
		return;
	}

	std::sort(markedContainers.begin(), markedContainers.end(), [](const auto& lhs, const auto& rhs) {
		return lhs.first < rhs.first;
	});

	for (const auto& entry : markedContainers) {
		const uint8_t cid = entry.first;
		Container* container = entry.second;
		if (!container || cid > 0xF || container->getTopParent() != this) {
			continue;
		}

		if (getContainerID(container) != -1) {
			continue;
		}

		addContainer(cid, container);
		sendContainer(cid, container, container->hasParent(), getContainerIndex(cid));
	}
...
void Player::onCreatureAppear(Creature* creature, bool isLogin)
{
	...
	if (const Player* seenPlayer = creature->getPlayer()) {
		g_playerShop.onCreatureAppear(this, seenPlayer);
	}
...
void Player::onRemoveCreature(Creature* creature, bool isLogout)
{
	...
	if (creature == this) {
		DispatcherPhaseMetricsTimer cleanupTimer(DispatcherMetricsPhase::LOGOUT_CLEANUP);
		if (isLogout) {
		...
		if (asynchronousLogout) {
			...
			if (g_playerIOManager.enqueueLogout(this,
					[playerName](bool saved, const std::string& error) {
						if (!saved) {
							std::cout << "Error while saving player asynchronously: "
							          << playerName;
							if (!error.empty()) {
								std::cout << " (" << error << ')';
							}
						...
						g_game.recordCleanSavePlayerResult(saved);
					})) {
				return;
			}
		}

		// This fallback is reached only when no PREPARE write could have reached
		// the service. Disable the historical unbounded transient-error retry loop
		// for this operation so a MariaDB outage cannot trap the Dispatcher.
		std::unique_ptr<DatabaseRetryLimitScope> boundedFallbackRetries;
		if (asynchronousLogout) {
			boundedFallbackRetries = std::make_unique<DatabaseRetryLimitScope>(0);
		}
		DispatcherPhaseMetricsTimer onlineStatusTimer(DispatcherMetricsPhase::LOGOUT_ONLINE_STATUS);
...
void Player::setNextRuneActionTask(SchedulerTask* task, bool resetIdleTime /*= true */)
{
	if (runeActionTaskEvent != 0) {
		g_scheduler.stopEvent(runeActionTaskEvent);
		runeActionTaskEvent = 0;
	}

	if (task) {
		runeActionTaskEvent = g_scheduler.addEvent(task);
		if (resetIdleTime) {
			this->resetIdleTime();
		}
	}
...
{
	g_playerShop.cancel(this, "death", true);
...
void Player::removeList()
{
	...
	DispatcherPhaseMetricsTimer vipNotifyTimer(DispatcherMetricsPhase::LOGOUT_VIP_NOTIFY);
...
void Player::addList()
{
	DispatcherPhaseMetricsTimer vipNotifyTimer(DispatcherMetricsPhase::LOGIN_VIP_NOTIFY);
	for (const auto& it : g_game.getPlayers()) {
...
bool Player::setAttackedCreature(Creature* creature)
{
	...
	if (creature) {
		...
		if (classicSpeed) {
			...
			const int64_t elapsed = lastAttack == 0 ? attackSpeed : now - lastAttack;
			if (elapsed >= attackSpeed) {
				nextClassicAttackCheck = 0;
				g_dispatcher.addTask(createTask(std::bind(&Game::checkCreatureAttack, &g_game, getID())));
			} else {
				const uint32_t delay = std::max<uint32_t>(SCHEDULER_MINTICKS, attackSpeed - elapsed);
				const int64_t scheduledAt = now + delay;
				if (nextClassicAttackCheck == 0 || nextClassicAttackCheck <= now || nextClassicAttackCheck > scheduledAt) {
					nextClassicAttackCheck = scheduledAt;
					g_scheduler.addEvent(createSchedulerTask(delay, std::bind(&Game::checkCreatureAttack, &g_game, getID())));
				}
			}
		} else {
			g_dispatcher.addTask(createTask(std::bind(&Game::checkCreatureAttack, &g_game, getID())));
...
void Player::doAttacking(uint32_t)
{
	...
	if ((OTSYS_TIME() - lastAttack) >= getAttackSpeed()) {
		...
		if (!classicSpeed) {
			...
			nextClassicAttackCheck = OTSYS_TIME() + delay;
		}
...
bool Player::onKilledCreature(Creature* target, bool lastHit/* = true*/)
{
	...
	if (lastHit) {
		if (const Monster* targetMonster = target->getMonster()) {
			g_game.recordBestiaryKill(*this, *targetMonster);
		}
	}
