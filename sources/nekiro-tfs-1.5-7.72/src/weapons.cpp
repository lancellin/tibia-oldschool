...
namespace
{
	bool isKnightMeleeFormulaVocation(const Player* player)
	{
		const uint16_t vocationId = player->getVocation()->getId();
		return vocationId == 4 || vocationId == 8;
	}

	float getOldschoolAttackModeFactor(float attackFactor)
	{
		if (attackFactor >= 1.9f) {
			return 0.5f;
		}
	...
	int32_t getOldschoolKnightMaxDamage(int32_t level, int32_t skill, int32_t attackValue, float attackModeFactor)
	{
		const double baseDamage = (level / 5.0) + (attackValue * 1.5) + (((skill * skill) / 1620.0) * attackValue);
		return static_cast<int32_t>(std::floor(baseDamage * attackModeFactor));
	}

	int32_t getOldschoolKnightMinDamage(int32_t level, int32_t skill, int32_t attackValue, float attackModeFactor)
	{
		const double maxBaseDamage = (level / 5.0) + (attackValue * 1.5) + (((skill * skill) / 1620.0) * attackValue);
		const double minBaseDamage = (level / 4.0) + (maxBaseDamage * 0.18);
		return static_cast<int32_t>(std::floor(minBaseDamage * attackModeFactor));
	}

	int32_t getOldschoolDistanceMaxDamage(int32_t level, int32_t skill, int32_t attackValue)
	{
		const double baseDamage = (level / 4.0) + 10.0 + (attackValue * ((skill / 15.0) + (std::pow(skill, 1.5) / 3100.0)));
		return static_cast<int32_t>(std::floor(baseDamage));
	}

	int32_t getOldschoolDistanceMinDamage(int32_t level, int32_t attackValue)
	{
		const double baseDamage = (level / 3.0) + attackValue;
		return static_cast<int32_t>(std::floor(baseDamage));
	}
}
...
bool Weapon::useFist(Player* player, Creature* target)
{
	...
	damage.primary.maxValue = -maxDamage;
...
void Weapon::internalUseWeapon(Player* player, Item* item, Creature* target, int32_t damageModifier) const
{
	if (scripted) {
		...
		damage.primary.maxValue = (getWeaponDamage(player, target, item, true) * damageModifier) / 100;
...
int32_t WeaponMelee::getWeaponDamage(const Player* player, const Creature*, const Item* item, bool maxDamage /*= false*/) const
{
	...
	if (attackValue > 16 && isKnightMeleeFormulaVocation(player)) {
		const float attackModeFactor = getOldschoolAttackModeFactor(attackFactor);
		const int32_t minValue = getOldschoolKnightMinDamage(player->getLevel(), attackSkill, attackValue, attackModeFactor);
		const int32_t maxValue = getOldschoolKnightMaxDamage(player->getLevel(), attackSkill, attackValue, attackModeFactor);

		if (maxDamage) {
			return -maxValue;
		}

		return -normal_random(std::min(minValue, maxValue), maxValue);
	}
...
int32_t WeaponDistance::getWeaponDamage(const Player* player, const Creature* target, const Item* item, bool maxDamage /*= false*/) const
{
	...
	if (attackValue > 10) {
		const float attackModeFactor = getOldschoolAttackModeFactor(attackFactor);
		const int32_t minValue = static_cast<int32_t>(std::floor(getOldschoolDistanceMinDamage(player->getLevel(), attackValue) * attackModeFactor));
		const int32_t maxValue = static_cast<int32_t>(std::floor(getOldschoolDistanceMaxDamage(player->getLevel(), attackSkill, attackValue) * attackModeFactor));
		if (maxDamage) {
			return -maxValue;
		}
