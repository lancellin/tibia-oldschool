...
enum class PlayerCharmState : uint8_t {
	LOCKED = 0,
	UNLOCKED = 1,
	ACTIVE = 2,
};
...
static constexpr uint8_t CHARM_SAVAGE_BLOW = 1;
static constexpr uint8_t CHARM_VAMPIRIC_EMBRACE = 2;
static constexpr uint8_t CHARM_VOIDS_CALL = 3;
static constexpr uint8_t CHARM_DODGE = 4;
...
struct CharmDefinition {
	...
	uint16_t unlockCost;
	uint8_t iconIndex;
	CharmEffectType effectType;
	uint8_t chance;
	uint8_t amountPercent;
	uint8_t criticalDamageMinPercent;
	uint8_t criticalDamageMaxPercent;
	int16_t aoeReductionPercent;
};

struct CharacterBonuses {
	std::array<int32_t, SKILL_LAST + 1> skillBonus = {};
	uint16_t totalCriticalChance = 0;
	uint16_t equipmentCriticalChance = 0;
	uint16_t equipmentCriticalDamagePercent = 0;
	uint16_t charmCriticalChance = 0;
	uint8_t charmCriticalDamageMinPercent = 0;
	uint8_t charmCriticalDamageMaxPercent = 0;
	uint16_t hitChance = 0;
	uint16_t equipmentLifeLeechChance = 0;
	uint16_t equipmentLifeLeechAmount = 0;
	uint16_t equipmentManaLeechChance = 0;
	uint16_t equipmentManaLeechAmount = 0;
	uint16_t charmLifeLeechChance = 0;
	uint16_t charmLifeLeechAmount = 0;
	uint16_t charmManaLeechChance = 0;
	uint16_t charmManaLeechAmount = 0;
	uint16_t charmManaLeechAreaEfficiency = 100;
	uint16_t charmDodgeChance = 0;
};
...
class Player final : public Creature, public Cylinder
{
	public:
		...
		PlayerCharmState getCharmState(uint8_t charmId) const;
		bool loadCharmStatesFromDatabase();
		bool setCharmState(uint8_t charmId, PlayerCharmState state, bool persist = true);
		bool unlockCharm(uint8_t charmId);
		...
		const std::map<uint32_t, DepotLocker_ptr>& getLoadedDepotLockers() const {
			return depotLockerMap;
		}
		const std::map<uint32_t, DepotChest*>& getLoadedDepotChests() const {
			return depotChests;
		}
		...
		int32_t getDefenseReduction(Creature* attacker, CombatType_t combatType) const override;
		int32_t getArmorReduction(Creature* attacker, CombatType_t combatType) const override;
		...
		void saveOpenContainerState();
		void restoreOpenContainerState();
		...
		void setPlayerShopState(uint8_t state) {
			playerShopState = state;
		}
		uint8_t getPlayerShopState() const {
			return playerShopState;
		}
		bool isInPlayerShop() const {
			return playerShopState != 0;
		}
	...
	private:
		...
		uint32_t deferredGuildId = 0;
		uint32_t deferredGuildRankId = 0;
		...
		int64_t nextClassicAttackCheck = 0;
		...
		uint32_t runeActionTaskEvent = 0;
		...
		uint32_t guid = 0;
		uint32_t playerIOReservationId = 0;
		uint32_t windowTextId = 0;
		uint32_t editListId = 0;
		uint32_t mana = 0;
		uint8_t playerShopState = 0;
		uint32_t manaMax = 0;
