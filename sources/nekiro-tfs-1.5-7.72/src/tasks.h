...
class Task
{
	public:
		...
		uint64_t getQueueWaitNanoseconds(std::chrono::steady_clock::time_point now) const {
			return static_cast<uint64_t>(
				std::chrono::duration_cast<std::chrono::nanoseconds>(now - queuedAt).count());
		}
...
class Dispatcher : public ThreadHolder<Dispatcher> {
	public:
		...
		uint64_t getExecutedNanoseconds() const {
			return executedNanoseconds.load(std::memory_order_relaxed);
		}
	...
	private:
		...
		std::atomic<uint64_t> executedNanoseconds{0};
		bool executionTimingEnabled = false;
		bool dispatcherMetricsTimingEnabled = false;
};
