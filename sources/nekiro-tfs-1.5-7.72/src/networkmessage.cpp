...
void NetworkMessage::addItem(const Item* item)
{
	...
	if (it.isContainer()) {
		// Container type 4 is the OTClient loot-highlight marker.
		const Tile* tile = item->getTile();
		const bool isUnopenedCorpseOnGround = tile && item->getParent() == tile &&
			item->getCustomAttribute(ITEM_CUSTOM_ATTRIBUTE_LOOT_UNOPENED);
		addByte(isUnopenedCorpseOnGround ? 4 : 0);
	}
}
