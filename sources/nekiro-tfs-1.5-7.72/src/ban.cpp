...
bool isConnectionThrottleBypassedForLoadTest()
{
	static const bool bypass = []() {
		const char* value = std::getenv("TFS_LOAD_TEST_BYPASS_CONNECTION_THROTTLE");
...
{
	if (isConnectionThrottleBypassedForLoadTest()) {
		return true;
	}
