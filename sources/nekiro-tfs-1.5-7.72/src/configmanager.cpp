...
bool ConfigManager::load()
{
	...
	if (!loaded) { //info that must be loaded one time (unless we reset the modules involved)
		...
		string[PLAYER_IO_SERVICE_HOST] = getGlobalString(L, "playerIoServiceHost", "127.0.0.1");
	...
	boolean[PLAYER_IO_SERVICE_ENABLED] = getGlobalBoolean(L, "playerIoServiceEnabled", false);
	...
	string[TOTP_KEY] = getGlobalString(L, "totp_key", "");
	...
	integer[RATE_SPAWN_TIME] = std::max<int32_t>(1, getGlobalNumber(L, "rateSpawnTime", 1));
	integer[BLOCKED_SPAWN_RETRY_INTERVAL] = getGlobalNumber(L, "spawnBlockedRetryInterval", 10);
	...
	integer[FLOOR_PERSISTENCE_WORLD_ID] = std::max<int32_t>(1, getGlobalNumber(L, "floorPersistenceWorldId", 1));
	integer[FLOOR_PERSISTENCE_GENERATION_ID] = std::max<int32_t>(1, getGlobalNumber(L, "floorPersistenceGenerationId", 1));
	integer[FLOOR_PERSISTENCE_SNAPSHOT_DEBOUNCE_MS] = std::max<int32_t>(1000, getGlobalNumber(L, "floorPersistenceSnapshotDebounceMs", 15000));
	integer[FLOOR_PERSISTENCE_SNAPSHOT_MAX_DELAY_MS] = std::max<int32_t>(1000, getGlobalNumber(L, "floorPersistenceSnapshotMaxDelayMs", 60000));
	integer[FLOOR_PERSISTENCE_SNAPSHOT_RETRY_MS] = std::max<int32_t>(1000, getGlobalNumber(L, "floorPersistenceSnapshotRetryMs", 5000));
	integer[FLOOR_PERSISTENCE_SNAPSHOT_BATCH_SIZE] = std::max<int32_t>(1, getGlobalNumber(L, "floorPersistenceSnapshotBatchSize", 32));
