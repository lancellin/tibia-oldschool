...
int32_t getMaximumDamageValue(int32_t first, int32_t second)
{
	return std::abs(first) >= std::abs(second) ? first : second;
}

TextColor_t getCriticalTextColor()
{
	static constexpr std::array<TextColor_t, 5> colors = {
		TEXTCOLOR_YELLOW,
		TEXTCOLOR_ORANGE,
		TEXTCOLOR_RED,
		TEXTCOLOR_PASTELRED,
	...
	return colors[uniform_random(0, static_cast<int32_t>(colors.size() - 1))];
}

bool canApplyCharmCritical(const Player* player, const Creature* target, const CombatDamage& damage)
{
	if (!player || !target || damage.critical || damage.primary.type == COMBAT_HEALING) {
		return false;
	}
	...
	return damage.primary.value < 0 && damage.primary.maxValue < 0;
}

void applyCharmCritical(Player* player, const Creature* target, CombatDamage& damage)
{
	if (!canApplyCharmCritical(player, target, damage)) {
		return;
	}

	const CharacterBonuses& bonuses = player->getCharacterBonuses();
	if (bonuses.charmCriticalChance == 0 || uniform_random(1, 100) > bonuses.charmCriticalChance) {
		return;
	}

	const int32_t primaryMaximum = std::abs(damage.primary.maxValue);
	const int32_t secondaryMaximum = std::abs(damage.secondary.maxValue);
	const int32_t maximumHit = primaryMaximum + secondaryMaximum;
	if (maximumHit <= 0) {
		return;
	}

	const uint8_t bonusPercent = uniform_random(
		bonuses.charmCriticalDamageMinPercent,
		bonuses.charmCriticalDamageMaxPercent);
	int32_t extraDamage = static_cast<int32_t>(std::floor(maximumHit * (bonusPercent / 100.0)));
	if (Combat::isPlayerCombat(target)) {
		extraDamage = static_cast<int32_t>(std::floor(extraDamage * 0.5));
	}

	damage.primary.value = -(primaryMaximum + extraDamage);
	damage.secondary.value = -secondaryMaximum;
	damage.critical = true;
	g_game.addColoredText(ColoredText("CRITICAL!", target->getPosition(), getCriticalTextColor()));

	std::cout << "[CharmCritical] player=" << player->getName()
			  << " target=" << target->getName()
			  << " maxHit=" << maximumHit
			  << " bonusPercent=" << static_cast<uint32_t>(bonusPercent)
			  << " extraDamage=" << extraDamage
			  << " finalPrimary=" << damage.primary.value
			  << " finalSecondary=" << damage.secondary.value
			  << std::endl;
}

bool applyCharmDodge(Creature* caster, Creature* target, const CombatDamage& damage)
{
	Player* targetPlayer = target ? target->getPlayer() : nullptr;
	if (!caster || !targetPlayer || caster == target || damage.leeched ||
	...
	const uint16_t dodgeChance = targetPlayer->getCharacterBonuses().charmDodgeChance;
	if (dodgeChance == 0 || uniform_random(1, 100) > dodgeChance) {
		return false;
	}

	g_game.addColoredText(ColoredText("DODGE!", targetPlayer->getPosition(), TEXTCOLOR_WHITE_EXP));
	std::cout << "[CharmDodge] player=" << targetPlayer->getName()
			  << " attacker=" << caster->getName()
			  << " chance=" << dodgeChance << '%'
			  << " primary=" << std::abs(damage.primary.value)
			  << " secondary=" << std::abs(damage.secondary.value)
			  << " origin=" << static_cast<uint32_t>(damage.origin)
			  << std::endl;
	return true;
}

struct CharmLeechTotals {
	int64_t healthPercentUnits = 0;
	int64_t manaPercentUnits = 0;
};

void accumulateCharmLeech(Player* player, const Creature* target, int32_t damageDealt, bool areaAttack, CharmLeechTotals& totals)
{
	if (!player || !target || damageDealt <= 0) {
		return;
	}

	const CharacterBonuses& bonuses = player->getCharacterBonuses();
	if (!target->getPlayer() && bonuses.charmLifeLeechChance > 0 && bonuses.charmLifeLeechAmount > 0 &&
		uniform_random(1, 100) <= bonuses.charmLifeLeechChance) {
		totals.healthPercentUnits += static_cast<int64_t>(damageDealt) * bonuses.charmLifeLeechAmount;
		std::cout << "[CharmLeech][Vampiric Embrace] player=" << player->getName()
				  << " target=" << target->getName()
				  << " chance=" << bonuses.charmLifeLeechChance << '%'
				  << " damage=" << damageDealt
				  << " area=" << (areaAttack ? "yes" : "no")
				  << " contribution=" << damageDealt * (bonuses.charmLifeLeechAmount / 100.0)
				  << " HP" << std::endl;
	}

	if (!target->getPlayer() && !target->isSummon() && bonuses.charmManaLeechChance > 0 && bonuses.charmManaLeechAmount > 0 &&
		uniform_random(1, 100) <= bonuses.charmManaLeechChance) {
		const uint16_t efficiency = areaAttack ? bonuses.charmManaLeechAreaEfficiency : 100;
		totals.manaPercentUnits += static_cast<int64_t>(damageDealt) * bonuses.charmManaLeechAmount * efficiency;
		std::cout << "[CharmLeech][Void's Call] player=" << player->getName()
				  << " target=" << target->getName()
				  << " chance=" << bonuses.charmManaLeechChance << '%'
				  << " damage=" << damageDealt
				  << " area=" << (areaAttack ? "yes" : "no")
				  << " efficiency=" << efficiency << '%'
				  << " contribution=" << damageDealt * (bonuses.charmManaLeechAmount / 100.0) * (efficiency / 100.0)
				  << " mana" << std::endl;
	}
}

void applyCharmLeechTotals(Player* player, const CharmLeechTotals& totals)
{
	if (!player) {
		return;
	}
	...
	const int64_t healthToRecover = (totals.healthPercentUnits + 50) / 100;
	if (healthToRecover > 0 && player->getHealth() < player->getMaxHealth()) {
		const int32_t healthBefore = player->getHealth();
		CombatDamage leechCombat;
		leechCombat.primary.type = COMBAT_HEALING;
		leechCombat.primary.value = static_cast<int32_t>(std::min<int64_t>(healthToRecover, player->getMaxHealth() - healthBefore));
		leechCombat.origin = ORIGIN_NONE;
		leechCombat.leeched = true;
		g_game.combatChangeHealth(nullptr, player, leechCombat);

		const int32_t recoveredHealth = player->getHealth() - healthBefore;
		if (recoveredHealth > 0) {
			g_game.addColoredText(ColoredText(std::to_string(recoveredHealth), position, TEXTCOLOR_LIGHTGREEN));
			player->sendMagicEffect(position, CONST_ME_MAGIC_RED);
		}
	}

	const int64_t manaToRecover = (totals.manaPercentUnits + 5000) / 10000;
	if (manaToRecover > 0 && player->getMana() < player->getMaxMana()) {
		const int32_t manaBefore = player->getMana();
		CombatDamage leechCombat;
		leechCombat.primary.type = COMBAT_HEALING;
		leechCombat.primary.value = static_cast<int32_t>(std::min<int64_t>(manaToRecover, player->getMaxMana() - manaBefore));
		leechCombat.origin = ORIGIN_NONE;
		leechCombat.leeched = true;
		g_game.combatChangeMana(nullptr, player, leechCombat);

		const int32_t recoveredMana = player->getMana() - manaBefore;
		if (recoveredMana > 0) {
			g_game.addColoredText(ColoredText(std::to_string(recoveredMana), position, TEXTCOLOR_MAYABLUE));
			player->sendMagicEffect(position, CONST_ME_MAGIC_BLUE);
		}
	}
}
...
CombatDamage Combat::getCombatDamage(Creature* creature, Creature* target) const
{
	...
	if (formulaType == COMBAT_FORMULA_DAMAGE) {
		const int32_t minValue = static_cast<int32_t>(mina);
		const int32_t maxValue = static_cast<int32_t>(maxa);
		damage.primary.value = normal_random(minValue, maxValue);
		damage.primary.maxValue = getMaximumDamageValue(minValue, maxValue);
	} else if (creature) {
		int32_t min, max;
		if (creature->getCombatValues(min, max)) {
			damage.primary.value = normal_random(min, max);
			damage.primary.maxValue = getMaximumDamageValue(min, max);
		} else if (Player* player = creature->getPlayer()) {
			if (params.valueCallback) {
				params.valueCallback->getMinMaxValues(player, damage);
			} else if (formulaType == COMBAT_FORMULA_LEVELMAGIC) {
				int32_t levelFormula = player->getLevel() * 2 + player->getMagicLevel() * 3;
				const int32_t minValue = std::fma(levelFormula, mina, minb);
				const int32_t maxValue = std::fma(levelFormula, maxa, maxb);
				damage.primary.value = normal_random(minValue, maxValue);
				damage.primary.maxValue = getMaximumDamageValue(minValue, maxValue);
				...
				if (weapon) {
					const int32_t minValue = minb;
					const int32_t maxValue = std::fma(weapon->getWeaponDamage(player, target, tool, true), maxa, maxb);
					damage.primary.value = normal_random(minValue, maxValue);
					damage.primary.maxValue = getMaximumDamageValue(minValue, maxValue);
					damage.secondary.type = weapon->getElementType();
					damage.secondary.value = weapon->getElementDamage(player, target, tool);
					damage.secondary.maxValue = damage.secondary.value;
				} else {
					damage.primary.value = normal_random(minb, maxb);
					damage.primary.maxValue = getMaximumDamageValue(minb, maxb);
				}
			}
		}
	}
...
void Combat::doTargetCombat(Creature* caster, Creature* target, CombatDamage& damage, const CombatParams& params)
{
	...
	if (damage.primary.type != COMBAT_MANADRAIN) {
		applyCharmCritical(casterPlayer, target, damage);
		...
		if (applyCharmDodge(caster, target, damage)) {
			return;
		}
	...
	if (success) {
		...
		if (!damage.leeched && damage.primary.type != COMBAT_HEALING && casterPlayer && damage.origin != ORIGIN_CONDITION) {
			...
			if (damage.primary.type != COMBAT_MANADRAIN) {
				CharmLeechTotals charmLeechTotals;
				accumulateCharmLeech(casterPlayer, target, totalDamage, false, charmLeechTotals);
				applyCharmLeechTotals(casterPlayer, charmLeechTotals);
			}
		}
...
void Combat::doAreaCombat(Creature* caster, const Position& position, const AreaCombat* area, CombatDamage& damage, const CombatParams& params)
{
	...
	for (Creature* creature : toDamageCreatures) {
		...
		applyCharmCritical(casterPlayer, creature, damageCopy);
		...
		if (damageCopy.primary.type != COMBAT_MANADRAIN) {
			...
			if (applyCharmDodge(caster, creature, damageCopy)) {
				continue;
			}
		...
		if (success) {
			...
			if (casterPlayer && !damage.leeched && damage.primary.type != COMBAT_HEALING && damage.origin != ORIGIN_CONDITION) {
				...
				if (damage.primary.type != COMBAT_MANADRAIN) {
					accumulateCharmLeech(casterPlayer, creature, totalDamage, true, charmLeechTotals);
				}
			}
...
void ValueCallback::getMinMaxValues(Player* player, CombatDamage& damage) const
{
	...
	if (lua_pcall(L, parameters, 2, 0) != 0) {
		...
		const int32_t minValue = LuaScriptInterface::getNumber<int32_t>(L, -2);
		const int32_t maxValue = LuaScriptInterface::getNumber<int32_t>(L, -1);
		damage.primary.value = normal_random(minValue, maxValue);
		damage.primary.maxValue = getMaximumDamageValue(minValue, maxValue);
		lua_pop(L, 2);
	}
