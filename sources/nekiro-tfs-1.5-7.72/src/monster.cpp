...
namespace {
	constexpr int64_t FOLLOW_PATH_UPDATE_INTERVAL = 150;
}
                             ...
                             const Tile* oldTile, const Position& oldPos, bool teleport)
{
	...
	if (creature == this) {
		...
		if (creature == followCreature && hasFollowPath) {
			walkUpdateTicks = 0;
			forceUpdateFollowPath = false;

			const int64_t now = OTSYS_TIME();
			if (now >= nextFollowPathUpdate) {
				nextFollowPathUpdate = now + FOLLOW_PATH_UPDATE_INTERVAL;
