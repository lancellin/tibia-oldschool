	...
	if (!lootDrop && getMonster()) {
		...
		if (corpse) {
			if (getMonster()) {
				...
				corpse->setCustomAttribute(creatureCorpseAttribute, true);
			}
			if (getMonster() && corpse->getContainer()) {
				std::string unopenedAttribute = ITEM_CUSTOM_ATTRIBUTE_LOOT_UNOPENED;
		...
		Tile* corpseTile = corpse ? corpse->getTile() : nullptr;
		if (corpse && getPlayer() && corpseTile && corpse->getParent() == corpseTile && !dynamic_cast<HouseTile*>(corpseTile)) {
			std::string playerCorpseAttribute = ITEM_CUSTOM_ATTRIBUTE_FLOOR_PLAYER_CORPSE;
			corpse->setCustomAttribute(playerCorpseAttribute, true);
			std::string deathBundleAttribute = ITEM_CUSTOM_ATTRIBUTE_FLOOR_DEATH_BUNDLE;
			corpse->setCustomAttribute(deathBundleAttribute, true);
			g_game.markFloorTileDirty(*corpseTile, FLOOR_DIRTY_DEATH_BUNDLE, FLOOR_DIRTY_ORIGIN_PLAYER_DEATH);
		}
	}
...
int32_t Creature::getDefenseReduction(Creature*, CombatType_t) const
{
...
int32_t Creature::getArmorReduction(Creature*, CombatType_t) const
{
...
int64_t Creature::getStepDuration() const
{
	...
	//int64_t stepDuration = std::ceil(duration / 10) * 10;
