...
class DatabaseRetryLimitScope
{
	public:
		explicit DatabaseRetryLimitScope(uint32_t maxTransientRetries);
		~DatabaseRetryLimitScope();

		DatabaseRetryLimitScope(const DatabaseRetryLimitScope&) = delete;
		DatabaseRetryLimitScope& operator=(const DatabaseRetryLimitScope&) = delete;

	private:
		std::optional<uint32_t> previousLimit;
};
...
class DBResult
{
	public:
		...
		DBResult(std::vector<std::string> columns,
		...
		T getNumber(const std::string& s) const
		{
			...
			T data;
			try {
				data = boost::lexical_cast<T>(*value);
	...
	private:
		...
		mutable std::string mysqlValueBuffer;
		std::vector<std::vector<std::optional<std::string>>> memoryRows;
		size_t memoryRowIndex = 0;
