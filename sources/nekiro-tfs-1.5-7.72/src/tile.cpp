...
void Tile::addThing(int32_t, Thing* thing)
{
	...
	if (creature) {
		g_game.map.onCreatureChanged(getPosition(), creature->getPlayer() != nullptr);
		...
		FloorDirtyReason_t dirtyReason = FLOOR_DIRTY_ITEM_ADD;
		item->setParent(this);
		...
		if (itemType.isGroundTile()) {
			if (ground == nullptr) {
				...
				dirtyReason = FLOOR_DIRTY_ITEM_REPLACE;
		...
		g_game.markFloorTileDirty(*this, dirtyReason);
	}
}
...
void Tile::updateThing(Thing* thing, uint16_t itemId, uint32_t count)
{
	...
	g_game.markFloorTileDirty(*this, FLOOR_DIRTY_ITEM_UPDATE);
}
...
void Tile::replaceThing(uint32_t index, Thing* thing)
{
	...
	if (isInserted) {
		...
		g_game.markFloorTileDirty(*this, FLOOR_DIRTY_ITEM_REPLACE);
		return /*RETURNVALUE_NOERROR*/;
	}
}
...
void Tile::removeThing(Thing* thing, uint32_t count)
{
	...
	if (item == ground) {
		...
		g_game.markFloorTileDirty(*this, FLOOR_DIRTY_ITEM_REMOVE);
		return;
	}
