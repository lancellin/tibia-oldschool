...
namespace {
thread_local std::optional<uint32_t> databaseTransientRetryLimit;

bool consumeDatabaseTransientRetry(uint32_t& retries)
{
	if (databaseTransientRetryLimit && retries >= *databaseTransientRetryLimit) {
		return false;
	}
	++retries;
	return true;
...
DatabaseRetryLimitScope::DatabaseRetryLimitScope(uint32_t maxTransientRetries) :
	previousLimit(databaseTransientRetryLimit)
{
	databaseTransientRetryLimit = maxTransientRetries;
}

DatabaseRetryLimitScope::~DatabaseRetryLimitScope()
{
	databaseTransientRetryLimit = previousLimit;
}
...
bool Database::connect()
{
	...
	// Local dev MariaDB on Windows often runs without TLS enabled.
...
bool Database::beginTransaction()
{
	if (PlayerIORemoteDatabaseScope* remote = PlayerIORemoteDatabaseScope::current()) {
		return remote->beginTransaction();
	}
...
{
	if (PlayerIORemoteDatabaseScope* remote = PlayerIORemoteDatabaseScope::current()) {
		return remote->executeQuery(query);
	}
...
DBResult::DBResult(std::vector<std::string> columns,
	std::vector<std::vector<std::optional<std::string>>> rows) :
	memoryRows(std::move(rows))
{
	for (size_t i = 0; i < columns.size(); ++i) {
		listNames[columns[i]] = i;
	}
}
...
const std::string* DBResult::getValue(size_t index) const
{
	if (handle) {
		if (!row || row[index] == nullptr) {
			return nullptr;
		}
		mysqlValueBuffer = row[index];
		return &mysqlValueBuffer;
	}

	if (memoryRowIndex >= memoryRows.size() || index >= memoryRows[memoryRowIndex].size()) {
		return nullptr;
	}
	const auto& value = memoryRows[memoryRowIndex][index];
...
std::string DBResult::getString(const std::string& s) const
{
	...
	if (!handle) {
		const std::string* value = getValue(it->second);
...
const char* DBResult::getStream(const std::string& s, unsigned long& size) const
{
	...
	if (!handle) {
		...
		size = static_cast<unsigned long>(value->size());
		return value->data();
	}
...
{
	return handle ? row != nullptr : memoryRowIndex < memoryRows.size();
}
...
bool DBResult::next()
{
	...
	++memoryRowIndex;
	return memoryRowIndex < memoryRows.size();
}
