...
namespace {
	void markContainerFloorDirty(Container& container, FloorDirtyReason_t reason)
	{
		Cylinder* topParent = container.getTopParent();
		if (topParent && g_game.isFloorDirtyPlayerMutationActive()) {
			Item* rootItem = topParent->getItem();
			if (rootItem && rootItem->removeCustomAttribute(ITEM_CUSTOM_ATTRIBUTE_FLOOR_DEATH_BUNDLE)) {
				rootItem->markAsPlayerMovedForFloorPersistence();
			}
		}
		...
		// The player's virtual DepotLocker is temporarily parented to the map
		// tile of the physical depot. Mutations inside the locker, its DepotChest
		// or any nested container belong to player persistence, not floor state.
		// Containers actually dropped on top of the depot tile do not have a
		// DepotLocker as their top parent and continue to mark that tile dirty.
		Item* topItem = topParent->getItem();
		Container* topContainer = topItem ? topItem->getContainer() : nullptr;
		if (topContainer && topContainer->getDepotLocker()) {
			return;
		}

		const Tile* tile = container.getTile();
		if (tile) {
			g_game.markFloorTileDirty(*tile, reason);
		}
	}
}
...
void Container::invalidateFloorPersistenceIdentifiedSubtree()
{
	for (Container* container = this; container; container = container->getParentContainer()) {
		container->floorPersistenceSubtreeIdentified = false;
	}
}

void Container::invalidateFloorPersistenceSubtreeIfNeeded(const Item& item)
{
	if (!g_game.isFloorDirtyPlayerMutationActive() && !item.isFloorPersistenceSubtreeReady()) {
		invalidateFloorPersistenceIdentifiedSubtree();
	}
}
...
{
	invalidateFloorPersistenceSubtreeIfNeeded(*item);
...
std::string Container::getLootMessageContent() const
{
	std::ostringstream os;
	bool firstItem = true;
	for (ContainerIterator it = iterator(); it.hasNext(); it.advance()) {
		...
		Container* childContainer = item->getContainer();
		if (childContainer && !childContainer->empty()) {
			continue;
		}
		...
		int64_t lootDifficulty = 0;
		if (const auto* difficultyAttribute = item->getCustomAttribute(ITEM_CUSTOM_ATTRIBUTE_LOOT_DIFFICULTY)) {
			if (const auto* difficulty = boost::get<int64_t>(&difficultyAttribute->value)) {
				lootDifficulty = std::max<int64_t>(0, std::min<int64_t>(5, *difficulty));
			}
		}
		os << '{' << itemType.clientId << '|' << lootDifficulty << '|' << item->getNameDescription() << '}';
	}
...
void Container::addThing(int32_t index, Thing* thing)
{
	...
	markContainerFloorDirty(*this, FLOOR_DIRTY_CONTAINER_ADD);
}
...
void Container::updateThing(Thing* thing, uint16_t itemId, uint32_t count)
{
	...
	markContainerFloorDirty(*this, FLOOR_DIRTY_CONTAINER_UPDATE);
}
...
void Container::removeThing(Thing* thing, uint32_t count)
{
	...
	if (item->isStackable() && count != item->getItemCount()) {
		...
		markContainerFloorDirty(*this, FLOOR_DIRTY_CONTAINER_REMOVE);
	}
}
