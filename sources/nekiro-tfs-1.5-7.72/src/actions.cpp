...
ReturnValue Actions::internalUseItem(Player* player, const Position& pos, uint8_t index, Item* item, bool isHotkey)
{
	...
	if (Container* container = item->getContainer()) {
		...
		if (oldContainerId == -1) {
			if (const auto* monsterNameAttribute = openContainer->getCustomAttribute(ITEM_CUSTOM_ATTRIBUTE_LOOT_MONSTER_NAME)) {
				if (openContainer->removeCustomAttribute(ITEM_CUSTOM_ATTRIBUTE_LOOT_UNOPENED)) {
					if (const auto* monsterName = boost::get<std::string>(&monsterNameAttribute->value)) {
						player->sendTextMessage(MESSAGE_LOOT,
							fmt::format("Loot of {}: {}", *monsterName, openContainer->getLootMessageContent()));
					}

					Tile* tile = openContainer->getTile();
					if (tile && openContainer->getParent() == tile) {
						SpectatorVec spectators;
						g_game.map.getSpectators(spectators, tile->getPosition(), true, true);
						for (Creature* spectator : spectators) {
							if (Player* spectatorPlayer = spectator->getPlayer()) {
								spectatorPlayer->sendUpdateTileItem(tile, tile->getPosition(), openContainer);
							}
						}
					}
				}
...
static uint32_t getRuneActionDelay(const Item* item)
{
	...
	switch (item->getID()) {
		...
		case 2273: // Ultimate Healing Rune
			return 1000;
		default:
			return g_config.getNumber(ConfigManager::EX_ACTIONS_DELAY_INTERVAL);
	}
}
...
{
	const uint32_t runeDelay = getRuneActionDelay(item);
	if (runeDelay != 0) {
		player->setNextRuneAction(OTSYS_TIME() + runeDelay);
	}
