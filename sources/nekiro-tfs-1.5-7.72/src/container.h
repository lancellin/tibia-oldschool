...
class DepotLocker;
class StoreInbox;
class FloorPersistenceSerializer;
...
class Container : public Item, public Cylinder
{
	public:
		...
		std::string getLootMessageContent() const;
		...
		uint32_t getSerializationCount() const {
			return serializationCount;
		}

		ContainerIterator iterator() const;
		bool hasFloorPersistenceIdentifiedSubtree() const {
			return floorPersistenceSubtreeIdentified;
		}
		void setFloorPersistenceIdentifiedSubtree(bool identified) {
			floorPersistenceSubtreeIdentified = identified;
		}
		void invalidateFloorPersistenceIdentifiedSubtree();
	...
	private:
		...
		void invalidateFloorPersistenceSubtreeIfNeeded(const Item& item);

		friend class ContainerIterator;
		friend class IOMapSerialize;
		friend class FloorPersistenceSerializer;
};
