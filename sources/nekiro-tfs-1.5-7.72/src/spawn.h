...
struct spawnBlock_t {
	...
	int64_t lastSpawn;
	int64_t nextSpawnAt = 0;
	int64_t blockedRetryAt = 0;
	uint32_t interval;
	Direction direction;
};
...
class Spawn
{
	...
	int64_t checkSpawnAt = 0;
	uint64_t checkSpawnGeneration = 0;
	...
	static uint32_t getBlockedSpawnRetryInterval();
	static uint32_t getDynamicSpawnInterval(uint32_t baseInterval);
	void markSpawnPending(uint32_t spawnId, int64_t now);
	void scheduleSpawnCheck(uint32_t delay);
	void scheduleNextSpawnCheck();
	SpawnResult spawnMonster(uint32_t spawnId, const spawnBlock_t& sb, bool startup = false);
