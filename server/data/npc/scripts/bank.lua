...
local function getMoneySnapshot(player)
	return {
		inventory = player:getMoney(),
		bank = player:getBankBalance(),
		total = player:getMoney() + player:getBankBalance()
	}
end

local function getOfflineTransferSnapshot(targetInfo)
	return {
		inventory = "offline",
		bank = targetInfo.balance,
		total = "offline"
	}
end

local function appendBankLog(line)
	local file = io.open("data/logs/banking.log", "a")
	if not file then
		return
	end
...
local function formatMoneySnapshot(label, name, snapshot)
	return string.format(
		"%s=%s{inventory=%s,bank=%s,total=%s}",
		label,
		name,
		tostring(snapshot.inventory),
		tostring(snapshot.bank),
		tostring(snapshot.total)
	)
end

local function logBankOperation(operation, actorName, beforeActor, afterActor, targetName, beforeTarget, afterTarget, amount)
	local parts = {
		string.format("[%s]", os.date("%d/%m/%Y %H:%M:%S")),
		string.format("operation=%s", operation),
		string.format("amount=%s", tostring(amount)),
		formatMoneySnapshot("actor_before", actorName, beforeActor),
		formatMoneySnapshot("actor_after", actorName, afterActor)
	}

	if targetName and beforeTarget and afterTarget then
		table.insert(parts, formatMoneySnapshot("target_before", targetName, beforeTarget))
		table.insert(parts, formatMoneySnapshot("target_after", targetName, afterTarget))
	end

	appendBankLog(table.concat(parts, " | "))
end

local function persistMoneyChange(player, targetInfo)
	player:save()
	if targetInfo then
		local targetPlayer = Player(targetInfo.guid)
...
local function creatureSayCallback(cid, type, msg)
	...
	elseif npcHandler.topic[cid] == topicList.DEPOSIT_CONSENT then
		if msgcontains(msg, "yes") then
			if player:getMoney() >= tonumber(count[cid]) then
				local beforeActor = getMoneySnapshot(player)
				player:depositMoney(count[cid])
				persistMoneyChange(player)
				logBankOperation("deposit", player:getName(), beforeActor, getMoneySnapshot(player), nil, nil, nil, count[cid])
		...
		if msgcontains(msg, "yes") then
			if player:canCarryMoney(count[cid]) then
				...
				if not player:withdrawMoney(count[cid]) then
					...
					persistMoneyChange(player)
					logBankOperation("withdraw", player:getName(), beforeActor, getMoneySnapshot(player), nil, nil, nil, count[cid])
	...
	elseif npcHandler.topic[cid] == topicList.TRANSFER_PLAYER_CONSENT then
		if msgcontains(msg, "yes") then
			local beforeActor = getMoneySnapshot(player)
			local targetPlayer = Player(transfer[cid].guid)
			local beforeTarget = targetPlayer and getMoneySnapshot(targetPlayer) or getOfflineTransferSnapshot(transfer[cid])
			if not player:transferMoneyTo(transfer[cid], count[cid]) then
				npcHandler:say("You cannot transfer money to this account.", cid)
			else
				persistMoneyChange(player, transfer[cid])
				local afterTarget
				if targetPlayer then
					afterTarget = getMoneySnapshot(targetPlayer)
				else
					afterTarget = {
						inventory = "offline",
						bank = beforeTarget.bank + count[cid],
						total = "offline"
					}
				end
				logBankOperation("transfer", player:getName(), beforeActor, getMoneySnapshot(player), transfer[cid].name, beforeTarget, afterTarget, count[cid])
	...
	elseif npcHandler.topic[cid] == topicList.CHANGE_GOLD_PLATINUM_CONSENT then
		if msgcontains(msg, "yes") then
			...
			if player:removeItem(ITEM_GOLD_COIN, count[cid] * 100) then
				...
				persistMoneyChange(player)
				logBankOperation("change_gold_to_platinum", player:getName(), beforeActor, getMoneySnapshot(player), nil, nil, nil, count[cid] * 100)
		...
		if msgcontains(msg, "yes") then
			if player:getFreeCapacity() >= getMoneyWeight(count[cid]) then
				...
				if player:removeItem(ITEM_PLATINUM_COIN, count[cid]) then
					...
					persistMoneyChange(player)
					logBankOperation("change_platinum_to_gold", player:getName(), beforeActor, getMoneySnapshot(player), nil, nil, nil, count[cid])
					npcHandler:say("Here you are.", cid)
				else
					npcHandler:say("Sorry, you do not have enough platinum coins.", cid)
				end
	...
	elseif npcHandler.topic[cid] == topicList.CHANGE_PLATINUM_CRYSTAL_CONSENT then
		if msgcontains(msg, "yes") then
			...
			if player:removeItem(ITEM_PLATINUM_COIN, count[cid] * 100) then
				...
				persistMoneyChange(player)
				logBankOperation("change_platinum_to_crystal", player:getName(), beforeActor, getMoneySnapshot(player), nil, nil, nil, count[cid] * 100)
				npcHandler:say("Here you are.", cid)
			else
				npcHandler:say("Sorry, you do not have enough platinum coins.", cid)
			end
		...
		if msgcontains(msg, "yes") then
			if player:getFreeCapacity() >= getMoneyWeight(count[cid]) then
				...
				if player:removeItem(ITEM_CRYSTAL_COIN, count[cid]) then
					...
					persistMoneyChange(player)
					logBankOperation("change_crystal_to_platinum", player:getName(), beforeActor, getMoneySnapshot(player), nil, nil, nil, count[cid])
					npcHandler:say("Here you are.", cid)
				else
					npcHandler:say("Sorry, you do not have enough crystal coins.", cid)
				end
