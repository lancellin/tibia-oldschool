...
local paradoxStartStorage = 6667
local paradoxMathStorage = 6668
...
local function creatureSayCallback(cid, type, msg)
	...
	if (msgcontains(msg, "numbers") or msgcontains(msg, "math") or msgcontains(msg, "mathemagic")) and player:getStorageValue(paradoxStartStorage) == 1 then
	...
	elseif msgcontains(msg, "yes") and npcHandler.topic[cid] == topic.MATH_FINISH then
		...
		player:setStorageValue(paradoxMathStorage, 1)
...
keywordHandler:addKeyword({"power"}, StdModule.say, {
	npcHandler = npcHandler,
	text = "Power. Hmmm. Once while we were crossing the mountains together a man named Aureus said to me that parcels are equal to power. Any idea what that meant?"
})
...
keywordHandler:addKeyword({"riddle"}, StdModule.say, {
	npcHandler = npcHandler,
	text = "Great riddle, isn´t it? If you can tell me the correct answer, I will give you something. Hehehe!"
})
