...
if msgcontains(msg, 'story') and npcHandler:isFocused(cid) then
...
elseif msgcontains(msg, 'fields of glory') and npcHandler:isFocused(cid) then
	npcHandler:say("Ok, sit down and listen. Back in the early days, one of the ancestors of our king Tibianus III wanted to build the best city in whole of Tibia.", 1)
	talk_state = 2
	
elseif talk_state == 2 and msgcontains(msg, 'ancestor') and npcHandler:isFocused(cid) then
npcHandler:say("Please forgive me. I forgot his name. I'm not that young anymore.", 1)
talk_state = 2

elseif talk_state == 2 and msgcontains(msg, 'city') and npcHandler:isFocused(cid) then
...
elseif talk_state == 3 and msgcontains(msg, 'soldier') and npcHandler:isFocused(cid) then
npcHandler:say("It was the elite of the whole army. They were called the Red Legion (also known as the Bloody Legion).", 1)
talk_state = 3

elseif talk_state == 3 and msgcontains(msg, 'orc') and npcHandler:isFocused(cid) then
npcHandler:say("The orcs attacked the workers from time to time and so they disturbed the works on the city.", 1)
talk_state = 4

elseif talk_state == 3 and msgcontains(msg, 'work harder') and npcHandler:isFocused(cid) then
npcHandler:say("The soldiers treated them like slaves.", 1)
talk_state = 4

elseif talk_state == 4 and msgcontains(msg, 'slave') and npcHandler:isFocused(cid) then
npcHandler:say("You dont know what a slave is? I really hope that you will never have to make this experience.", 1)
talk_state = 3

elseif talk_state == 4 and msgcontains(msg, 'works') and npcHandler:isFocused(cid) then
...
elseif talk_state == 5 and msgcontains(msg, 'rebel') and npcHandler:isFocused(cid) then
...
elseif talk_state == 6 and msgcontains(msg, 'friends') and npcHandler:isFocused(cid) then
...
elseif talk_state == 7 and msgcontains(msg, 'cruelty') and npcHandler:isFocused(cid) then
npcHandler:say("The soldiers treated the workers like slaves.", 1)
talk_state = 7

elseif talk_state == 7 and msgcontains(msg, 'plan') and npcHandler:isFocused(cid) then
...
elseif talk_state == 8 and msgcontains(msg, 'island') and npcHandler:isFocused(cid) then
...
elseif talk_state == 8 and msgcontains(msg, 'attack') and npcHandler:isFocused(cid) then
npcHandler:say("The General of the Red Legion became very angry about these attacks and after some months he stroke back!", 1)
talk_state = 9

elseif talk_state == 9 and msgcontains(msg, 'stroke') and npcHandler:isFocused(cid) then
...
elseif talk_state == 10 and msgcontains(msg, 'back') and npcHandler:isFocused(cid) then
...
elseif talk_state == 10 and  msgcontains(msg, 'walk') and npcHandler:isFocused(cid) then
npcHandler:story("It is said that the orcish shamans cursed the Red Legion. ...", 1)
npcHandler:story("Nobody knows. But one third of the soldiers died by a disease on the way back. ...", 5)
npcHandler:story("And the orcs wanted to take revenge, and after some days they stroke back! ...", 9)
npcHandler:story("The orcs and many allied cyclopses and minotaurs from all over Tibia came to avenge their friends, and they killed nearly all workers and soldiers in the castle. ...", 13)
...
elseif talk_state == 11 and  msgcontains(msg, 'help') and npcHandler:isFocused(cid) then
...
elseif talk_state == 12 and  msgcontains(msg, 'trick') and npcHandler:isFocused(cid) then
...
elseif talk_state == 12 and  msgcontains(msg, 'destruction') and npcHandler:isFocused(cid) then
npcHandler:story("They used the same trick as against the Red Legion and the orcs started to fight their non-orcish-allies. ...", 1)
npcHandler:story("After a bloody long fight the orcs went back to their cities. The city of Carlin was rescued. ...", 5)
npcHandler:story("Since then, a woman has always been ruling over Carlin and this statue was made to remind us of their great tactics against the orcs and the Red Legion. ...", 9)
npcHandler:say("So that was the story of Carlin and these Fields of Glory. I hope you liked it. *smiles*", 13)
