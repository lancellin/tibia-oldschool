...
local function addTravelKeyword(keyword, cost, destination)
	local travelKeyword = keywordHandler:addKeyword({keyword}, StdModule.say, {
		...
		onlyFocus = true,
		text = string.format('Do you seek a passage to %s for %d gold?', titleCase(keyword), cost)
	})
		...
		travelKeyword:addChildKeyword({'no'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'We would like to serve you some time.', reset = true})
end
...
function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, 'yes') and talkState == 3 then
		...
		if isPremium(cid) == true then
			if player and not player:isPzLocked() then
	...
	elseif msgcontains(msg, 'no') and talkState == 2 then
