...
function onCreatureSay(cid, type, msg)
	if not npcHandler:isFocused(cid) and (msgcontains(msg, 'hail general') or msgcontains(msg, 'salutations general')) then
...
elseif talk_state == 2 and msgcontains(msg, 'yes') and npcHandler:isFocused(cid) then
	doSendMagicEffect(getCreaturePosition(getNpcCid(  )), 13)
	doSendMagicEffect(getPlayerPosition(cid), 15)
	doAddCondition(cid, fire)
	npcHandler:say("Take this!", 0.5)
	talk_state = 0
elseif talk_state == 2 and msgcontains(msg, '') and npcHandler:isFocused(cid) then
