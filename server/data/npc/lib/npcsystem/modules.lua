...
if Modules == nil then
	...
	SHOP_TRADEREQUEST = {"trade", "offer", "shop"}
	...
	function StdModule.travel(cid, message, keywords, parameters, node)
		...
		return npcHandler:executeTravelRoute(cid, parameters, {requireFocus = true, costMultiplier = 1, releaseFocus = true, resetNpc = true})
	end
	...
	function ShopModule:callbackOnBuy(cid, itemid, subType, amount, ignoreCap, inBackpacks)
		local backpackId = NPC_SHOP_BACKPACK or ITEM_SHOPPING_BAG or 1988
		...
		if inBackpacks then
			if ItemType(itemid):isStackable() then
				backpackCount = math.ceil(amount / (ItemType(backpackId):getCapacity() * 100))
			else
				backpackCount = math.ceil(amount / ItemType(backpackId):getCapacity())
			end
			totalCost = totalCost + (backpackCount * 20)
		end
		...
		if a < amount then
			...
			parseInfo[TAG_TOTALCOST] = (a * shopItem.buy) + (b * 20)
			msg = self.npcHandler:parseMessage(msg, parseInfo)
			player:sendCancelMessage(msg)
