...
if not KeywordHandler then
	...
	function KeywordNode:addChildKeyword(keywords, callback, parameters)
		...
		local node = self:addChildKeywordNode(new)
		if StdModule and callback == StdModule.travel and parameters and parameters.npcHandler and parameters.destination then
			if self.parameters and type(self.parameters.text) == 'string' then
				local lowerText = self.parameters.text:lower()
				if not lowerText:find("{yes}", 1, true) and not lowerText:find("{no}", 1, true) then
					self.parameters.text = self.parameters.text .. " {Yes} or {No}."
				end
			end
			parameters.npcHandler:registerTravelDestination(self, parameters)
		end
		return node
	end
