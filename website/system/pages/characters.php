...
WHERE killers.death_id = '".$death['id']."' ORDER BY killers.final_hit DESC, killers.id ASC")->fetchAll();
	...
	if($player->isDeleted()) {
		$character_status = 'deleted';
	}
	else if(isset($bannedUntil) && $bannedUntil !== '') {
		$character_status = 'Banished';
	}
	...
	$search_errors[] = 'Character <b>' . htmlspecialchars($name) . '</b> does not exist or has been deleted.';
