...
if($save)
{
	$limiter = new RateLimit('account_create', 5, 60);
	$limiter->enabled = true;
	$limiter->load();
	$create_ip = get_browser_real_ip();
	if($limiter->exceeded($create_ip)) {
		$errors['rate'] = 'Too many accounts created from your IP address. Please try again later.';
	}
	else if(!Captcha::verify($_POST['captcha'] ?? '')) {
		$errors['captcha'] = 'The captcha code is incorrect or has expired.';
	}

	if(empty($errors) && !config('account_login_by_email')) {
		if(USE_ACCOUNT_NAME) {
	...
	if(empty($errors))
	{
		...
		else
		{
			if(setting('core.account_create_auto_login')) {
				...
				// account creation already enforced captcha for this request
				$GLOBALS['skip_login_captcha'] = true;
