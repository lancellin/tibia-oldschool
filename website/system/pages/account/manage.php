...
$players = array();
$cancelable_ids = array();
/** @var OTS_Players_List $account_players */
$account_players = $account_logged->getPlayersList();
$account_players->orderBy('id');
foreach($account_players as $account_player) {
	if($account_player->isDeleted()) {
		// deleted characters disappear from the account after the 7-day
		// cancellation grace period; they are never removed from the database
		if(time() - $account_player->getCustomField('deletion') >= 604800) {
			continue;
		}
		$cancelable_ids[] = $account_player->getId();
	}
	$players[] = $account_player;
}
