...
// internal auto-login right after account creation already passed captcha
if(empty($GLOBALS['skip_login_captcha']) && !\MyAAC\Captcha::verify($_POST['captcha'] ?? '')) {
...
if(!empty($login_account) && !empty($login_password))
{
	...
	)
	{
		if (setting('core.account_mail_verify') && (int)$account_logged->getCustomField('email_verified') !== 1) {
			...
			$pwhash = encrypt((USE_ACCOUNT_SALT ? $account_logged->getCustomField('salt') : '') . $login_password);
			$want_admin = isset($_POST['admin']);

			if (\MyAAC\TwoFactor::secretFor($account_logged) !== null) {
				// 2FA enabled: a trusted device cookie skips the second step,
				// otherwise the user is sent to the TOTP form without being logged in.
				if (\MyAAC\TwoFactor::verifyTrustedCookie($account_logged->getId())) {
					if (!myaac_complete_login($account_logged, $pwhash, $remember_me, $want_admin)) {
						$errors[] = 'This account has no admin privileges.';
					}
				}
				else {
					\MyAAC\TwoFactor::setPending($account_logged->getId(), $remember_me, $want_admin, $pwhash);
					session_regenerate_id(true);
					header('Location: ' . getLink('account/twofactor'));
					exit;
				}
			}
			else {
