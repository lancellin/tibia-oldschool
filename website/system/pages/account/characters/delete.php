...
// characters are never removed from the database: deletion schedules a
// 7-day grace period during which the owner can cancel; afterwards the
// character only disappears from the account view (status "deleted").
$deletion_grace_seconds = 7 * 24 * 3600;

if(isset($_POST['action']) && $_POST['action'] == 'cancel') {
	$cancel_name = isset($_POST['delete_name']) ? stripslashes($_POST['delete_name']) : null;
	$errors = array();
	if(empty($cancel_name)) {
		$errors[] = 'Character name is empty.';
	}
	else {
		$cancel_player = new OTS_Player();
		$cancel_player->find($cancel_name);
		if(!$cancel_player->isLoaded() || $account_logged->getId() != $cancel_player->getAccount()->getId()) {
			$errors[] = 'Character <b>' . htmlspecialchars($cancel_name) . '</b> is not on your account.';
		}
		else if(!$cancel_player->isDeleted()) {
			$errors[] = 'This character is not scheduled for deletion.';
		}
		else if(time() - $cancel_player->getCustomField('deletion') >= $deletion_grace_seconds) {
			$errors[] = 'The cancellation period for this character has ended.';
		}
		else {
			$cancel_player->setCustomField('deletion', 0);
			$account_logged->logAction('Cancelled deletion of character <b>' . $cancel_player->getName() . '</b>.');
			$twig->display('success.html.twig', array(
				'title' => 'Deletion Cancelled',
				'description' => 'The character <b>' . $cancel_player->getName() . '</b> has been restored to your account.'
			));
			return;
		}
	}
...
if(isset($_POST['deletecharactersave']) && $_POST['deletecharactersave'] == 1) {
	...
	if(empty($errors)) {
		...
		if ($db->hasColumn('players', 'deletion'))
			$player->setCustomField('deletion', time());
		else
		...
		$twig->display('success.html.twig', [
			...
			'description' => 'The character <b>' . $player_name . '</b> has been deleted. It will disappear from your account in 7 days; until then you can cancel the deletion on the account management page.'
		]);
	}
}
